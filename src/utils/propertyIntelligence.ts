...
import { ASSET_PROFILE_PROPERTY_TYPES } from './calculateUaeQuote2026';
...
type CanonicalPropertyType = (typeof ASSET_PROFILE_PROPERTY_TYPES)[number];
...
const FOOD_COURT = S('food_court', 'Food court', 'ردهة طعام', 'amenity');
...
const GYM_SET = [GYM, RECEPTION, OFFICE, LOCKER, BATHROOM, STORAGE, LAUNDRY, SECURITY, ELECTRICAL, MECHANICAL, PLANT, PARKING, POOL, SPA];
...
export const PROPERTY_SPACE_CATALOG: Record<CanonicalPropertyType, SpaceDefinition[]> = {
  Villa: RESIDENTIAL,
  ...
  Office: OFFICE_SET,
  'Gym / Fitness Centre': GYM_SET,
  'Retail Center': RETAIL_SET,
  Mall: [...RETAIL_SET, RESTAURANT, KITCHEN, FOOD_COURT],
  Hotel: HOSPITALITY,
...
export function getSuggestedSpaces(propertyType: string): SpaceDefinition[] {
  ...
  return PROPERTY_SPACE_CATALOG[propertyType as CanonicalPropertyType].filter((item) => {
