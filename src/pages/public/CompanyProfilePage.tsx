...
const content = {
  en: {
    ...
    oneMinuteSubtitle: 'One app. Four clear journeys. Everyone knows what to do next.',
    roles: [
      ...
      ['Broker', 'Submit lead, track approval, and follow commission after the deal is verified.', 'Submit a lead'],
    ],
    ...
    proofItems: ['Licensed UAE company', 'Clear service flow', 'Photos for proof', 'GPS/location context', 'Time-stamped records', 'Tenant approval or dispute', 'Owner cost visibility', 'English and Arabic support'],
    flowTitle: 'The simple idea',
    flow: ['Report', 'Track', 'Prove', 'Approve'],
    sections: { simple: 'Simple Concept', trust: 'Trust Proof', coverage: 'Service Areas', contact: 'Start With BIN GROUP' },
    services: [
      ...
      ['Property Management', 'Owners see property status, costs, reports, and service history clearly, from one login, on demand.'],
      ['Technician Work', 'Field work is connected to job status, GPS location context, and before/after proof on every visit.'],
      ['Broker Growth', 'Property leads and commission progress stay visible and organized, from first lead to verified deal.'],
      ...
      ['Property Passport', 'Each property keeps its contracts, tickets, evidence, invoices, and history together — built for the day you sell, renew, or hand it over.'],
    ],
    contactText: 'Start with one property, one tenant request, or one maintenance contract. BIN GROUP keeps the process clear from the first click.',
  ...
  ar: {
    ...
    oneMinuteSubtitle: 'تطبيق واحد. أربع رحلات واضحة. كل شخص يعرف الخطوة التالية.',
    roles: [
      ...
      ['الوسيط', 'أرسل فرصة عقارية، تابع الموافقة، وتابع العمولة بعد تأكيد الصفقة.', 'أرسل فرصة'],
    ],
    ...
    proofItems: ['شركة مرخصة في الإمارات', 'مسار خدمة واضح', 'صور إثبات', 'سياق موقع GPS', 'سجلات بالوقت', 'موافقة أو اعتراض المستأجر', 'وضوح تكلفة المالك', 'دعم عربي وإنجليزي'],
    flowTitle: 'الفكرة ببساطة',
    flow: ['بلّغ', 'تابع', 'أثبت', 'وافق'],
    sections: { simple: 'الفكرة البسيطة', trust: 'إثبات الثقة', coverage: 'مناطق الخدمة', contact: 'ابدأ مع BIN GROUP' },
    services: [
      ['الصيانة', 'طلبات الخدمة تصبح تذاكر متتبعة بدل مكالمات ضائعة — كل عمل يُسند ويُرسل ويُغلق بصورة إثبات.'],
      ['إدارة العقار', 'المالك يرى حالة العقار والتكلفة والتقارير وسجل الخدمة بوضوح، من دخول واحد، في أي وقت.'],
      ['عمل الفني', 'العمل الميداني مرتبط بالحالة وسياق موقع GPS وصور قبل/بعد في كل زيارة.'],
      ['نمو الوسيط', 'الفرص العقارية ومتابعة العمولة تبقى واضحة ومنظمة، من أول فرصة حتى تأكيد الصفقة.'],
      ...
      ['جواز العقار', 'كل عقار يحتفظ بالعقود والتذاكر والإثبات والفواتير والتاريخ في مكان واحد — جاهز ليوم البيع أو التجديد أو التسليم.'],
    ],
    contactText: 'ابدأ بعقار واحد، طلب مستأجر واحد، أو عقد صيانة واحد. BIN GROUP تجعل العملية واضحة من أول ضغطة.',
...
export default function CompanyProfilePage() {
  ...
  const serviceIcons = [<Wrench size={22} />, <Building2 size={22} />, <Zap size={22} />, <Briefcase size={22} />, <Sparkles size={22} />, <Award size={22} />];
  ...
  return <Box sx={{ minHeight: '100vh', bgcolor: '#fff', color: ink, direction: isRTL ? 'rtl' : 'ltr', position: 'relative', overflowX: 'hidden' }}><BrandWatermark opacity={0.06} /><Box sx={{ position: 'relative', zIndex: 1 }}>
    ...
    <Section dark chip={c.flowTitle} title={c.flowTitle}><Grid container spacing={2}>{c.flow.map((step, i) => <Grid item xs={6} md={3} key={step}><Paper sx={{ p: 3, textAlign: 'center', borderRadius: 3, bgcolor: 'rgba(255,255,255,.05)', border: `1px solid ${alpha(gold, .2)}` }}><Typography sx={{ color: alpha(gold, .8), fontWeight: 950, fontSize: '.85rem' }}>0{i + 1}</Typography><Typography variant="h5" fontWeight={950} sx={{ color: '#fff', mt: 1 }}>{step}</Typography></Paper></Grid>)}</Grid></Section>
    <Section chip="WHAT WE DO" title="What We Do"><Grid container spacing={3}>{c.services.map(([title, body], i) => <Grid item xs={12} sm={6} md={4} key={title}><Card title={title} body={body} icon={serviceIcons[i]} /></Grid>)}</Grid></Section>
