...
const copy = {
  en: {
    ...
    desc: 'BIN GROUP gives serious UAE property owners a unified Home OS: property details, instant quote, contract selection, evidence-backed physical inspections, 15% mobilization only after required inspections, tenant registry, rent ledger waterfall, 5% management fee logic, technician dispatch, SLA timers, before-and-after proof, owner reports, and permanent property passport history.',
    primary: 'Start Property Details',
    companyProfileCta: 'Company Profile',
    quote: 'Get Instant Quote',
    login: 'Portal Login',
    whatsapp: 'WhatsApp BIN GROUP',
    flow: 'Owner journey: details → quote and contract → physical inspections → exact 15% → verified activation',
    ...
    propertyPathDesc: 'The owner starts with property details, service scope, documents, and submitted map/GPS evidence. BIN GROUP then creates one physical site visit for every property. Only after all required inspections are completed does the final verified commercial amount become due; the exact 15% payment is then verified before the remaining activation gates and dashboard unlock.',
    propertyDetails: [
    ...
    contractSteps: [
      ['1', 'Submit property details and documents', 'The five-page Owner flow captures the real asset profile, required documents, contract evidence, and Owner-submitted map/GPS evidence. Submitted Owner GPS is not yet dispatch-ready.'],
      ['2', 'BIN GROUP creates physical site visits', 'Admin creates one linked inspection for every submitted property. No 15% mobilization payment is collected before the required visits are completed.'],
      ['3', 'Complete evidence-backed inspections', 'Each property visit requires the protected GPS, timestamps, checklist, photo/PDF evidence, evidence hash, and Storage generation before it can be completed.'],
      ['4', 'Final terms and exact 15% become due', 'After every required inspection is complete, the server finalizes the commercial terms and exact 15% mobilization amount. Payment evidence must match the current verified amount and policy.'],
      ['5', 'Admin verification and activation', 'Admin verifies the permitted payment evidence and remaining activation prerequisites. Only then can the Owner dashboard unlock.'],
    ],
  ...
  ar: {
    ...
    desc: 'مجموعة بن تمنح ملاك العقارات في الإمارات نظام Home OS موحد: تفاصيل العقار، عرض السعر، اختيار العقد، فحوصات ميدانية موثقة، دفعة تفعيل 15٪ فقط بعد اكتمال الفحوصات المطلوبة، سجل المستأجرين، دفتر الإيجارات، منطق رسوم الإدارة 5٪، إرسال الفنيين، مؤقتات SLA، إثبات قبل وبعد، تقارير المالك، وسجل جواز العقار الدائم.',
    primary: 'ابدأ تفاصيل العقار',
    companyProfileCta: 'ملف الشركة',
    quote: 'احصل على عرض سعر فوري',
    login: 'دخول البوابة',
    whatsapp: 'تواصل عبر واتساب',
    flow: 'رحلة المالك: تفاصيل ← عرض وعقد ← فحوصات ميدانية ← 15٪ بالمبلغ الدقيق ← تفعيل موثق',
    ...
    propertyPathDesc: 'يبدأ المالك بتفاصيل العقار ونطاق الخدمة والمستندات وبيانات الموقع/GPS المقدمة. تنشئ BIN GROUP بعد ذلك زيارة ميدانية فعلية مرتبطة لكل عقار. لا تصبح الشروط التجارية النهائية ودفعة التفعيل الدقيقة 15٪ مستحقة إلا بعد اكتمال جميع الفحوصات المطلوبة، ثم تتحقق الإدارة من إثبات الدفع وبقية متطلبات التفعيل قبل فتح لوحة المالك.',
    propertyDetails: [
    ...
    contractSteps: [
      ['1', 'إرسال تفاصيل العقار والمستندات', 'يجمع مسار المالك المكوّن من خمس صفحات ملف الأصل الحقيقي والمستندات المطلوبة وإثبات العقد وبيانات الخريطة/GPS المقدمة من المالك. موقع المالك المقدم لا يصبح جاهزاً للإرسال التشغيلي بمجرد تقديمه.'],
      ['2', 'إنشاء الزيارات الميدانية بواسطة BIN GROUP', 'تنشئ الإدارة فحصاً مرتبطاً واحداً لكل عقار مقدم. لا يتم تحصيل دفعة التفعيل 15٪ قبل اكتمال الزيارات المطلوبة.'],
      ['3', 'إكمال الفحوصات الميدانية الموثقة', 'يتطلب كل فحص GPS المحمي والطوابع الزمنية وقائمة التحقق وصور/ملف PDF ودليل التجزئة وتوليد التخزين قبل إكماله.'],
      ['4', 'تثبيت الشروط واستحقاق 15٪ بالمبلغ الدقيق', 'بعد اكتمال جميع الفحوصات المطلوبة يثبت الخادم الشروط التجارية والمبلغ الدقيق لدفعة التفعيل 15٪. يجب أن يطابق إثبات الدفع المبلغ والسياسة الحاليين.'],
      ['5', 'تحقق الإدارة والتفعيل', 'تتحقق الإدارة من إثبات الدفع المسموح وبقية متطلبات التفعيل. عندها فقط يمكن فتح لوحة المالك.'],
    ],
...
function Trust() {
  const items = [
    ['15%', 'Due only after required property inspections'],
