...
const copy = {
  en: {
    ...
    desc: 'BIN GROUP gives serious UAE property owners a unified Home OS: property submission, BIN GROUP review, evidence-backed physical inspection, final server-verified quotation, Owner contract acceptance, payment verification, activation, tenant registry, maintenance operations, owner reports, and permanent property passport history.',
    primary: 'Start Property Details',
    companyProfileCta: 'Company Profile',
    quote: 'Start Property Review',
    login: 'Portal Login',
    whatsapp: 'WhatsApp BIN GROUP',
    flow: 'Owner journey: property submission → BIN GROUP review → site inspection → final quotation → contract → payment → activation',
    ...
    propertyPathDesc: 'The owner submits property details, service scope, documents, and untrusted map/GPS evidence. BIN GROUP reviews the dossier, creates the required physical site inspection, verifies the pricing inputs, issues the final server quotation, and then asks the Owner to accept and OTP-sign the contract. Only after that contract is signed does the exact 15% mobilisation payment become due; Admin verifies payment before activation and dashboard unlock.',
    propertyDetails: [
    ...
    contractSteps: [
      ['1', 'Submit property details and documents', 'The five-page Owner flow captures the real asset profile, required documents, and Owner-submitted map/GPS evidence. Submitted Owner GPS is not yet operationally trusted.'],
      ['2', 'BIN GROUP reviews the property dossier', 'Admin reviews the submitted documents, ownership context, property details, and location evidence before the property can move into physical verification.'],
      ['3', 'Complete the physical site inspection', 'Each required property visit records protected GPS, timestamps, checklist, verified pricing inputs, photo/PDF evidence, evidence hash, and Storage generation.'],
      ['4', 'Receive the final verified quotation', 'Only after all required inspections are complete does the server calculate the final commercial terms and exact 15% mobilisation amount from verified property facts.'],
      ['5', 'Accept and OTP-sign the final contract', 'The Owner reviews the final verified terms, completes the contract OTP signature, and receives the canonical server-generated signed PDF before payment becomes due.'],
      ['6', 'Pay the exact mobilisation amount', 'Payment evidence must match the current signed contract, verified quote hash, exact amount, and active payment policy.'],
      ['7', 'Admin verifies payment and activates', 'Admin verifies the permitted payment evidence and remaining activation prerequisites. Only then can the property and Owner dashboard become active.'],
    ],
    ...
    command: [
      ...
      ['Verified Quote', 'Final pricing follows required property inspection', 'QUOTE'],
    ...
    pricing: [['Maintenance', 'Custom Quote'], ['Management', '5% model'], ['Mobilization', '15% after required inspections'], ['Payment Plans', 'Monthly / Quarterly / Annual']],
  ...
  ar: {
    ...
    desc: 'مجموعة بن تمنح ملاك العقارات في الإمارات نظام Home OS موحد: تقديم العقار، مراجعة BIN GROUP، فحص ميداني موثق، عرض سعر نهائي محسوب على الخادم، قبول المالك للعقد، التحقق من الدفع، التفعيل، تشغيل الصيانة، تقارير المالك، وسجل جواز العقار الدائم.',
    primary: 'ابدأ تفاصيل العقار',
    companyProfileCta: 'ملف الشركة',
    quote: 'ابدأ مراجعة العقار',
    login: 'دخول البوابة',
    whatsapp: 'تواصل عبر واتساب',
    flow: 'رحلة المالك: تقديم العقار ← مراجعة BIN GROUP ← فحص ميداني ← عرض نهائي ← عقد ← دفع ← تفعيل',
    ...
    propertyPathDesc: 'يقدّم المالك تفاصيل العقار ونطاق الخدمة والمستندات وبيانات الموقع/GPS غير الموثقة تشغيلياً. تراجع BIN GROUP الملف ثم تنفذ الفحص الميداني المطلوب وتتحقق من مدخلات التسعير، وبعدها يصدر الخادم عرض السعر النهائي. يراجع المالك العقد النهائي ويوقّعه عبر OTP، وبعد التوقيع فقط تصبح دفعة التفعيل الدقيقة 15٪ مستحقة. تتحقق الإدارة من الدفع قبل التفعيل وفتح لوحة المالك.',
    propertyDetails: [
    ...
    contractSteps: [
      ['1', 'تقديم تفاصيل العقار والمستندات', 'يجمع مسار المالك ملف الأصل والمستندات المطلوبة وبيانات الخريطة/GPS المقدمة من المالك. هذه البيانات لا تصبح موثقة تشغيلياً بمجرد الإرسال.'],
      ['2', 'مراجعة ملف العقار بواسطة BIN GROUP', 'تراجع الإدارة المستندات وسياق الملكية وبيانات العقار والموقع قبل الانتقال إلى التحقق الميداني.'],
      ['3', 'إكمال الفحص الميداني', 'يسجل كل فحص GPS المحمي والطوابع الزمنية وقائمة التحقق ومدخلات التسعير المتحقق منها والصور/ملف PDF وتجزئة الدليل وتوليد التخزين.'],
      ['4', 'استلام عرض السعر النهائي الموثق', 'بعد اكتمال جميع الفحوصات المطلوبة يحسب الخادم الشروط التجارية النهائية ومبلغ التفعيل الدقيق 15٪ من حقائق العقار المتحقق منها.'],
      ['5', 'قبول العقد النهائي وتوقيعه عبر OTP', 'يراجع المالك الشروط النهائية ويوقّع العقد عبر OTP ويحصل على ملف PDF رسمي مولد من الخادم قبل استحقاق الدفع.'],
      ['6', 'دفع مبلغ التفعيل الدقيق', 'يجب أن يطابق إثبات الدفع العقد الموقع وعرض السعر الحالي والمبلغ الدقيق وسياسة الدفع الفعالة.'],
      ['7', 'تحقق الإدارة والتفعيل', 'تتحقق الإدارة من إثبات الدفع وبقية متطلبات التفعيل. عندها فقط يصبح العقار ولوحة المالك في حالة نشطة.'],
    ],
    ...
    command: [
      ...
      ['عرض سعر موثق', 'التسعير النهائي يأتي بعد الفحص المطلوب', 'سعر'],
    ...
    pricing: [['الصيانة', 'عرض مخصص'], ['الإدارة', 'نموذج 5٪'], ['التفعيل', '15٪ بعد الفحوصات المطلوبة'], ['الدفع', 'شهري / ربع سنوي / سنوي']],
