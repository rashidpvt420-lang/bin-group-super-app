...
import { Box, Container, LinearProgress, Stack, Step, StepLabel, Stepper, Typography, Button, Alert } from '@mui/material';
import { useNavigate } from 'react-router-dom';
import { ArrowLeft, Lock } from 'lucide-react';
import { useLanguage } from '../context/LanguageContext';
import { useOnboardingStore } from '../store/onboardingStore';
import { installSafeOnboardingPersistence } from '../store/onboardingPersistence';
...
// Company(1) | Property(2-4) | Service Plan(5) | Verification(6-8) | Contract & Payment(9-11)
...
export default function PropertyOnboardingPage() {
    ...
    const visibleStages = [
        ...
        readable(t('onboarding.verification'), label('Verification', 'التحقق')),
    ...
    React.useEffect(() => {
        installSafeOnboardingPersistence();
    }, []);
        ...
        <Box className="white-platinum-onboarding" dir={isRTL ? 'rtl' : 'ltr'} sx={{ minHeight: '100dvh', bgcolor: '#FFFFFF', color: '#111827', py: { xs: 2, md: 4 } }}>
            <Container maxWidth="xl">
                <Stack direction={isRTL ? 'row-reverse' : 'row'} justifyContent="space-between" alignItems="center" sx={{ mb: 3 }}>
                    <Button startIcon={<ArrowLeft size={18} style={{ transform: isRTL ? 'rotate(180deg)' : undefined }} />} onClick={() => navigate('/')} sx={{ color: '#B8932F', fontWeight: 900 }}>{readable(t('onboarding.back_home'), label('Back Home', 'الرجوع'))}</Button>
                    <Typography variant="caption" sx={{ color: '#B8932F', fontWeight: 900 }}>BIN GROUP ONBOARDING</Typography>
                </Stack>
                <Alert icon={<Lock size={18} />} severity="info" sx={{ mb: 3 }}>
                    {label(
                        'Your restricted owner login is created only to protect uploads and signatures. The dashboard stays locked until payment verification and explicit Admin approval.',
                        'يتم إنشاء دخول مقيّد للمالك فقط لحماية المستندات والتوقيع. تبقى لوحة التحكم مقفلة حتى التحقق من الدفع والموافقة الصريحة من الإدارة.'
                    )}
                </Alert>
