...
export function resolveOwnerFinancials(
  ...
  const mobilizationAmount = Number(contract?.quoteSnapshot?.activationDeposit ?? contract?.commercialSchedule?.mobilizationAmount ?? contract?.paymentSchedule?.mobilizationAmount ?? contract?.mobilizationAmount ?? contract?.depositAmount ?? 0);
