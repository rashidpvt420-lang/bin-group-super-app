...
function statusTone(status: string, balance: number, overdueDays: number) {
  ...
  if (status.includes('ACTIVE') || status.includes('PAID')) return '#10b981';
    ...
    <Paper sx={{ p: { xs: 2.25, md: 4 }, bgcolor: 'rgba(15,23,42,.58)', border: `1px solid ${alpha(binThemeTokens.gold, .18)}`, borderRadius: 5 }}>
      <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" alignItems={{ xs: 'flex-start', md: 'center' }} spacing={2} sx={{ mb: 3 }}>
        <Box>
          ...
          <Typography variant="h5" fontWeight={950} sx={{ color: '#fff' }}>Rent collection, payment proof and tenant balance control</Typography>
          <Typography variant="body2" sx={{ color: 'rgba(255,255,255,.58)', mt: .75 }}>Uses the resolved tenant ledger. No fake finance values are shown.</Typography>
        </Box>
        <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
          <Button startIcon={<Plus size={16} />} variant="contained" onClick={() => setOpen(true)} sx={{ bgcolor: binThemeTokens.gold, color: '#000', fontWeight: 950 }}>
            Record Rent Payment
          </Button>
          <Button startIcon={<Download size={16} />} variant="outlined" onClick={() => window.print()} sx={{ borderColor: binThemeTokens.gold, color: binThemeTokens.gold, fontWeight: 950 }}>
            Print Statement
          </Button>
        </Stack>
      </Stack>
      ...
      <TableContainer sx={{ border: '1px solid rgba(255,255,255,.08)', borderRadius: 3 }}>
        <Table size="small">
          ...
          <TableBody>
            {rows.map((row) => {
              ...
              return (
                <TableRow key={row.id} hover>
                  ...
                  <TableCell><Chip size="small" label={row.overdueDays > 0 ? `${row.overdueDays}D OVERDUE` : row.status.replace(/_/g, ' ')} sx={{ bgcolor: alpha(tone, .12), color: tone, fontWeight: 900 }} /></TableCell>
            ...
            {rows.length === 0 && (
              <TableRow>
                <TableCell colSpan={8} align="center" sx={{ py: 5, color: 'rgba(255,255,255,.42)', fontWeight: 800 }}>
                  No tenant ledger rows found yet. Add tenants, leases, occupancies or rent payment records to populate this table.
                </TableCell>
              </TableRow>
            )}
          </TableBody>
