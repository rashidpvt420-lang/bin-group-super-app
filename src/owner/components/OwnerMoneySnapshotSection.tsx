...
import { db, doc, getDownloadURL, ref, serverTimestamp, setDoc, storage, uploadBytes } from '../../lib/firebase';
...
export default function OwnerMoneySnapshotSection({ ledgerSummary, pendingPayments, properties, onRecordRentPayment }: Props) {
  ...
  useEffect(() => {
    ...
    setForm((current) => current.propertyId ? current : { ...current, ...emptyRentForm(properties) });
  }, [properties]);
  ...
  const selectProperty = (propertyId: string) => {
    ...
    setForm((current) => ({ ...current, propertyId, propertyName: propertyNameOf(property) || current.propertyName || 'Property' }));
  };
  ...
  return (
    <Paper sx={{ p: { xs: 2.25, md: 4 }, bgcolor: 'rgba(15,23,42,.58)', border: `1px solid ${alpha(binThemeTokens.gold, .18)}`, borderRadius: 5 }}>
      <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" alignItems={{ xs: 'flex-start', md: 'center' }} spacing={2} sx={{ mb: 3 }}>
        <Box>
          <Typography variant="overline" sx={{ color: binThemeTokens.gold, fontWeight: 950, letterSpacing: 3 }}>OWNER MONEY SNAPSHOT</Typography>
          <Typography variant="h5" fontWeight={950} sx={{ color: '#fff' }}>Rent collection, reference and tenant balance control</Typography>
          <Typography variant="body2" sx={{ color: 'rgba(255,255,255,.58)', mt: .75 }}>Rent submissions now create a payment verification item, ledger row, audit log, and notification reference.</Typography>
        </Box>
        <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
          <Button startIcon={<Plus size={16} />} variant="contained" onClick={() => setOpen(true)} sx={{ bgcolor: binThemeTokens.gold, color: '#000', fontWeight: 950 }}>Record Rent Payment</Button>
          <Button startIcon={<Download size={16} />} variant="outlined" onClick={() => window.print()} sx={{ borderColor: binThemeTokens.gold, color: binThemeTokens.gold, fontWeight: 950 }}>Print Statement</Button>
        </Stack>
      </Stack>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        {cards.map((card) => (
          <Grid item xs={12} sm={6} md={4} key={card.label}>
            <Paper sx={{ p: 2.25, bgcolor: 'rgba(255,255,255,.035)', border: `1px solid ${alpha(card.tone, .25)}`, borderRadius: 4 }}>
              <Box sx={{ color: card.tone, mb: 1 }}>{card.icon}</Box>
              <Typography variant="h6" fontWeight={950} sx={{ color: '#fff' }}>{card.value}</Typography>
              <Typography variant="caption" sx={{ color: 'rgba(255,255,255,.48)', fontWeight: 900, letterSpacing: 1 }}>{card.label.toUpperCase()}</Typography>
            </Paper>
          </Grid>
        ))}
      </Grid>
            ...
            {rows.map((row) => {
              ...
              return (
                <TableRow key={row.id} hover>
                  ...
                  <TableCell align="right" sx={{ color: row.balance > 0 ? '#ef4444' : '#10b981', fontWeight: 900 }}>{money(row.balance)}</TableCell>
                  <TableCell><Chip size="small" label={row.overdueDays > 0 ? `${row.overdueDays}D OVERDUE` : String(row.status || '').replace(/_/g, ' ')} sx={{ bgcolor: alpha(tone, .12), color: tone, fontWeight: 900 }} /></TableCell>
                  <TableCell sx={{ color: 'rgba(255,255,255,.72)' }}>{row.lastPaymentDate || 'Not recorded'}</TableCell>
                  <TableCell align="right"><Button size="small" variant="outlined" onClick={() => setSelectedRow(row)} sx={{ borderColor: alpha(binThemeTokens.gold, .45), color: binThemeTokens.gold, fontWeight: 900 }}>View</Button></TableCell>
                </TableRow>
              );
            })}
            {rows.length === 0 && (
              <TableRow>
                <TableCell colSpan={9} align="center" sx={{ py: 5, color: 'rgba(255,255,255,.42)', fontWeight: 800 }}>No tenant ledger rows found yet. Add tenants, leases, occupancies or rent payment records to populate this table.</TableCell>
              </TableRow>
            )}
          </TableBody>
        </Table>
      ...
      <Dialog open={open} onClose={() => setOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Stack spacing={2.25} sx={{ pt: 1 }}>
            ...
            <Button variant="outlined" component="label" sx={{ borderColor: alpha(binThemeTokens.gold, .45), color: binThemeTokens.gold, fontWeight: 900 }}>
              Attach reference file
              <input hidden type="file" accept="image/*,application/pdf" onChange={(event) => setReferenceFile(event.target.files?.[0] || null)} />
            </Button>
            {referenceFile && <Typography variant="caption" sx={{ color: 'rgba(255,255,255,.64)' }}>Selected: {referenceFile.name}</Typography>}
            <TextField label="Notes" value={form.notes} onChange={(event) => updateForm('notes', event.target.value)} fullWidth multiline minRows={3} />
          </Stack>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setOpen(false)}>Cancel</Button>
          <Button disabled={saving || !isRentRecordValid} onClick={submit} variant="contained" sx={{ bgcolor: binThemeTokens.gold, color: '#000', fontWeight: 950 }}>{saving ? 'Saving...' : 'Save rent record'}</Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={Boolean(selectedRow)} onClose={() => setSelectedRow(null)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Stack spacing={1.5} sx={{ pt: 1 }}>
            ...
            <Info label="Method" value={selectedRow?.method || selectedRow?.paymentMethod} />
            <Info label="Reference" value={selectedRow?.referenceId || selectedRow?.paymentReference} />
            <Info label="Reference file" value={selectedRow?.referenceFileName || selectedRow?.referenceFilePath} />
            {selectedRow?.referenceFileUrl && <Button variant="outlined" onClick={() => openExternalUrl(selectedRow.referenceFileUrl)} sx={{ borderColor: alpha(binThemeTokens.gold, .45), color: binThemeTokens.gold, fontWeight: 900 }}>Open reference file</Button>}
...
function Info({ label, value }: { label: string; value: any }) {
  return (
    <Paper sx={{ p: 1.5, bgcolor: 'rgba(15,23,42,.72)', border: '1px solid rgba(255,255,255,.08)' }}>
      <Typography variant="caption" sx={{ color: '#DAA520', fontWeight: 900 }}>{label.toUpperCase()}</Typography>
