  ...
  React.useEffect(() => {
    ...
    try {
      unsubs.push(onSnapshot(query(collection(db, 'maintenanceTickets'), where('ownerId', '==', ownerId), limit(150)), (snap) => {
        const openRows = snap.docs.map((docSnap) => docSnap.data()).filter((ticket) => OPEN_TICKET_STATUSES.has(String(ticket?.status || 'OPEN')) || OPEN_TICKET_STATUSES.has(String(ticket?.trackingStatus || '')));
    ...
    try {
      unsubs.push(onSnapshot(query(collection(db, 'owner_documents'), where('ownerId', '==', ownerId), limit(100)), (snap) => {
        ...
        partial.expiringDocuments = snap.docs.filter((docSnap) => {
          ...
          const ms = typeof expiry?.toDate === 'function' ? expiry.toDate().getTime() : new Date(expiry || 0).getTime();
