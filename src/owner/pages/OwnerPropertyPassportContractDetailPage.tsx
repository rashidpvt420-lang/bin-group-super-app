...
export default function OwnerPropertyPassportContractDetailPage() {
  ...
  const hasAnyLocationSignal = propertyLocation.locationQuality !== 'MISSING';
  const gpsTone = propertyLocation.hasExactCoordinates ? '#10b981' : hasAnyLocationSignal ? '#f59e0b' : '#ef4444';
  const gpsLabel = propertyLocation.hasExactCoordinates ? 'EXACT GPS PIN' : hasAnyLocationSignal ? 'APPROXIMATE GPS — REVIEW REQUIRED' : 'GPS NOT CONFIGURED';
        ...
        {activeTab === 'map' && (
          <Grid container spacing={3} sx={{ flexDirection: isRTL ? 'row-reverse' : 'row' }}>
            ...
            <Grid item xs={12} md={4}>
              <Stack spacing={2}>
                <Alert severity={propertyLocation.hasExactCoordinates ? 'success' : hasAnyLocationSignal ? 'warning' : 'error'} sx={{ bgcolor: alpha(gpsTone, 0.12), color: gpsTone, border: `1px solid ${alpha(gpsTone, 0.22)}`, py: 0.5 }}><Typography variant="caption" fontWeight="950">{gpsLabel}</Typography></Alert>
                <Metric label="Coordinates" value={propertyLocation.hasExactCoordinates ? `${propertyLocation.latitude?.toFixed?.(6)}, ${propertyLocation.longitude?.toFixed?.(6)}` : hasAnyLocationSignal ? (propertyLocation.address || propertyLocation.embedQuery || 'Approximate') : 'Missing'}><MapPin size={20} /></Metric>
                ...
                <Button disabled={!hasAnyLocationSignal} variant="contained" onClick={() => propertyLocation.googleMapsUrl && window.open(propertyLocation.googleMapsUrl, '_blank', 'noopener,noreferrer')} sx={{ bgcolor: hasAnyLocationSignal ? binThemeTokens.gold : 'rgba(255,255,255,0.08)', color: hasAnyLocationSignal ? '#111827' : 'rgba(255,255,255,0.35)', fontWeight: 950 }}>Open Google Maps</Button>
              </Stack>
            </Grid>
          </Grid>
        )}
