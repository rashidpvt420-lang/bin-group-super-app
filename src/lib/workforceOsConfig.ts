...
export const WORKFORCE_OS_MUST_HAVE_MODULES = [
  'heat_stress_exception',
  'eosb_final_settlement',
  'visa_permit_status',
] as const;

export type WorkforceOsComplianceStatus = 'valid' | 'expired' | 'warning' | 'missing';

export function getWorkforceComplianceFieldStatus(fieldName: string, expiryDateString?: string): WorkforceOsComplianceStatus {
  if (!expiryDateString) return 'missing';
  try {
