...
export function calculateBuildingHealth(data: {
    ...
    if (hvacScore < 60) recommendations.push("Predictive Maintenance Alert: Urgent chiller plant audit recommended due to efficiency drop.");
    if (plumbScore < 60) recommendations.push("High-pressure pump array showing fatigue. AI Property Recommendation: Install IoT Leak Detection Sensors.");
    if (data.age >= 10) recommendations.push("Predictive Maintenance Alert: Mandatory HVAC duct inspection for buildings 10+ years old.");
    if (overallScore < 70) recommendations.push("Consider upgrading to Premium PM for deeper asset preservation.");
