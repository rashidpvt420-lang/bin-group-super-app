                    ...
                    <Grid item xs={12} lg={4}>
                        <Paper sx={{ p: 4, borderRadius: 6, bgcolor: 'rgba(22, 22, 24, 0.8)', border: `2px solid ${binThemeTokens.gold}`, position: { lg: 'sticky' }, top: 24 }}>
                            ...
                            <Stack spacing={1.5} sx={{ mb: 3 }}>
                                ...
                                {(quote?.gradeMultiplier || 1) > 1 && <Box sx={{ display: 'flex', justifyContent: 'space-between', flexDirection: isRTL ? 'row-reverse' : 'row' }}><Typography variant="caption" color="rgba(255,255,255,0.55)">{ar ? 'علاوة الدرجة' : 'Asset Grade'}</Typography><Typography variant="caption" color={binThemeTokens.gold} fontWeight="700">{quote?.effectiveGrade} ({quote?.gradeMultiplier}×)</Typography></Box>}
