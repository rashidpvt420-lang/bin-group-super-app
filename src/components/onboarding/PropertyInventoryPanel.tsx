...
import {
  Bot, CheckCircle2, FileText, Minus, Plus, Sparkles, Upload, X,
} from 'lucide-react';
...
const PropertyInventoryPanel: React.FC<Props> = ({ property, onChange, ar, isRTL }) => {
  ...
  const updateInventory = (next: SpaceInventoryItem[]) => {
    onChange({
      spaceInventory: next,
      propertyIntelligence: calculatePropertyIntelligence({ ...property, spaceInventory: next }),
    });
  };
  ...
  const addCustomSpace = () => {
    ...
    if (!value) return;
    const id = `custom_${value.toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_|_$/g, '') || Date.now()}`;
  ...
  const uploadFloorPlan = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const fileRef = ref(storage, `temp_kyc/${uid}/floor_plans/${Date.now()}_${safeName}`);
  ...
  return (
    <Paper sx={{ p: { xs: 2.5, md: 4 }, mt: 4, borderRadius: 6, bgcolor: 'rgba(22,22,24,0.72)', border: `1px solid ${alpha(binThemeTokens.gold, 0.25)}` }}>
      ...
      <Stack direction={{ xs: 'column', sm: isRTL ? 'row-reverse' : 'row' }} spacing={1.5} alignItems={{ xs: 'stretch', sm: 'center' }}>
        <Button component="label" variant="outlined" startIcon={floorPlanUploading ? <CircularProgress size={16} /> : <Upload size={17} />} disabled={floorPlanUploading} sx={{ color: binThemeTokens.gold, borderColor: binThemeTokens.gold }}>
