...
export default function TenantCorrectionPanel({ residences }: { residences: Residence[] }) {
    ...
    return <Box data-testid="tenant-correction-panel" dir={isRTL ? 'rtl' : 'ltr'} sx={{ mt: 4 }}>
        <Paper sx={{ p: { xs: 2.5, md: 4 }, mb: 4, borderRadius: 5, bgcolor: 'rgba(22,22,24,.74)', border: '1px solid rgba(255,255,255,.07)' }}>
            ...
            {notice && <Alert data-testid={notice.type === 'success' ? 'tenant-correction-success' : 'tenant-correction-error'} severity={notice.type} sx={{ mb: 3 }} onClose={() => setNotice(null)}>{notice.text}</Alert>}
            <Grid container spacing={2}>
                <Grid item xs={12} md={6}><TextField data-testid="tenant-correction-field" select fullWidth label={copy('Record field', 'حقل السجل')} value={field} onChange={(event) => setField(event.target.value)}>{options.map((item) => <MenuItem key={item.value} value={item.value}>{copy(item.en, item.ar)}</MenuItem>)}</TextField></Grid>
                {selected.residence && <Grid item xs={12} md={6}><TextField data-testid="tenant-correction-residence" select fullWidth label={copy('Residence', 'السكن')} value={residenceId} onChange={(event) => setResidenceId(event.target.value)}>{residences.map((item) => <MenuItem key={item.id} value={item.id}>{copy('Unit', 'الوحدة')} {item.unitNumber || item.unit || item.id}</MenuItem>)}</TextField></Grid>}
                <Grid item xs={12} md={selected.residence ? 6 : 12}><TextField data-testid="tenant-correction-value" fullWidth type={selected.type || 'text'} label={copy('Correct value', 'القيمة الصحيحة')} value={requestedValue} onChange={(event) => setRequestedValue(event.target.value)} InputLabelProps={selected.type === 'date' ? { shrink: true } : undefined} /></Grid>
                <Grid item xs={12}><TextField data-testid="tenant-correction-reason" fullWidth multiline minRows={3} label={copy('Why is this correction needed?', 'لماذا يلزم هذا التصحيح؟')} value={reason} onChange={(event) => setReason(event.target.value)} inputProps={{ maxLength: 1000 }} /></Grid>
            </Grid>
            <Stack direction={isRTL ? 'row-reverse' : 'row'} justifyContent="flex-end" sx={{ mt: 3 }}><Button data-testid="tenant-correction-submit" variant="contained" disabled={submitting} startIcon={submitting ? <CircularProgress size={17} color="inherit" /> : <Send size={17} />} onClick={() => void submit()} sx={{ bgcolor: binThemeTokens.gold, color: '#000', fontWeight: 950 }}>{copy('Submit correction', 'إرسال التصحيح')}</Button></Stack>
        </Paper>

        <Paper data-testid="tenant-correction-history" sx={{ p: { xs: 2.5, md: 4 }, borderRadius: 5, bgcolor: 'rgba(22,22,24,.74)', border: '1px solid rgba(255,255,255,.07)' }}>
            ...
            {loading ? <Box data-testid="tenant-correction-loading" sx={{ textAlign: 'center', py: 4 }}><CircularProgress sx={{ color: binThemeTokens.gold }} /></Box> : requests.length ? <Stack spacing={2}>{requests.map((item) => <Paper data-testid={`tenant-correction-request-${item.id}`} key={item.id} sx={{ p: 2.5, bgcolor: 'rgba(0,0,0,.25)', borderRadius: 3 }}>
                <Stack direction={isRTL ? 'row-reverse' : 'row'} justifyContent="space-between"><Typography color="#FFF" fontWeight={900}>{copy(options.find((option) => option.value === item.field)?.en || item.field, options.find((option) => option.value === item.field)?.ar || item.field)}</Typography><Chip data-testid="tenant-correction-status" size="small" color={chipColor(item.status)} label={item.status.replaceAll('_', ' ')} /></Stack>
                <Divider sx={{ my: 2, borderColor: 'rgba(255,255,255,.07)' }} />
                <Grid container spacing={2}><Grid item xs={12} md={6}><Typography variant="caption" color="text.secondary">{copy('Current value', 'القيمة الحالية')}</Typography><Typography color="#FFF">{item.currentValue || '—'}</Typography></Grid><Grid item xs={12} md={6}><Typography variant="caption" color="text.secondary">{copy('Requested value', 'القيمة المطلوبة')}</Typography><Typography data-testid="tenant-correction-requested-value" color="#FFF" fontWeight={800}>{item.requestedValue}</Typography></Grid></Grid>
                ...
                <Stack data-testid="tenant-correction-events" sx={{ mt: 2 }}>{item.events.map((event) => <Typography key={event.id} variant="caption" color="text.secondary">{formatDate(event.createdAtMs)} · {event.eventType.replaceAll('_', ' ')}{event.reason ? ` · ${event.reason}` : ''}</Typography>)}</Stack>
            </Paper>)}</Stack> : <Typography data-testid="tenant-correction-empty" color="text.secondary">{copy('No correction requests submitted.', 'لم يتم إرسال طلبات تصحيح.')}</Typography>}
        </Paper>
    </Box>;
}
