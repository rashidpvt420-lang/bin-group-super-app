...
import { Box, Typography, Paper, Container, Grid, Button, alpha, Dialog, DialogTitle, DialogContent, DialogActions, TextField, MenuItem, Select, FormControl, InputLabel, CircularProgress, Stack, Card, CardContent, Alert } from '@mui/material';
...
export default function TenantAmenitiesPage() {
  ...
  const handleCancelBooking = async (bookingId: string) => {
    ...
    try {
      ...
      setActionMessage('Amenity booking cancelled successfully.');
    } catch (err) {
      console.error('Failed to cancel booking', err);
      setActionMessage('Failed to cancel booking. Please try again.');
    } finally {
      setCancelingId(null);
    }
  };
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 6, direction: isRTL ? 'rtl' : 'ltr' }}>
      ...
      {actionMessage && <Alert severity={actionMessage.startsWith('Failed') ? 'error' : 'success'} sx={{ mb: 3 }}>{actionMessage}</Alert>}
