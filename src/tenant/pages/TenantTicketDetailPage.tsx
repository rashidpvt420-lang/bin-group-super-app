...
import { Check, X, ChevronLeft, Calendar, AlertCircle, CheckCircle2, Info } from 'lucide-react';
...
export default function TenantTicketDetailPage() {
    ...
    const handleApprove = async () => {
        ...
        const cleanFeedback = feedback.trim() || tx('ticket.default_approve_feedback', 'Approved by tenant. Service completed successfully.');
        try {
    ...
    return (
        <Box sx={{ maxWidth: 900, mx: 'auto', pb: 10, direction: isRTL ? 'rtl' : 'ltr' }}>
            <Stack direction="row" alignItems="center" spacing={2} sx={{ mb: 4 }}>
                ...
                <Box>
                    <Typography variant="overline" sx={{ color: binThemeTokens.gold, fontWeight: 900, letterSpacing: 4 }}>{tx('ticket.record_label', 'TICKET RECORD')}</Typography>
            ...
            <Grid container spacing={4}>
                <Grid item xs={12} lg={8}>
                    <Paper sx={{ p: 4, mb: 4, bgcolor: 'rgba(22, 22, 24, 0.7)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 6 }}>
                        <Stack direction="row" justifyContent="space-between" alignItems="flex-start" sx={{ mb: 4 }}>
                            <Box>
                                <Typography variant="h5" fontWeight="950" color="#FFF">{ticket.category || ticket.complaintCategory || ticket.trade || tx('ticket.maintenance_request', 'Maintenance Request')}</Typography>
                            ...
                            <Chip label={String(ticket.status || 'OPEN').replace(/_/g, ' ')} sx={{ bgcolor: binThemeTokens.gold, color: '#000', fontWeight: 950, fontSize: '0.7rem' }} />
                        </Stack>
                        <Divider sx={{ mb: 4, borderColor: 'rgba(255,255,255,0.05)' }} />
                        <Stack spacing={3}>
                            <Box>
                                <Typography variant="caption" sx={{ color: binThemeTokens.gold, fontWeight: 900, letterSpacing: 1 }}>{tx('ticket.location_label', 'SPECIFIC LOCATION')}</Typography>
                            ...
                            <Box>
                                <Typography variant="caption" sx={{ color: binThemeTokens.gold, fontWeight: 900, letterSpacing: 1 }}>{tx('ticket.description_label', 'DESCRIPTION')}</Typography>
                                <Typography variant="body1" color="rgba(255,255,255,0.8)" sx={{ mt: 1, lineHeight: 1.7 }}>{ticket.description}</Typography>
                            </Box>
                            {ticket.photos && ticket.photos.length > 0 && (
                                <Box>
                                    <Typography variant="caption" sx={{ color: binThemeTokens.gold, fontWeight: 900, letterSpacing: 1, mb: 2, display: 'block' }}>{tx('ticket.submitted_photos', 'SUBMITTED PHOTOS')}</Typography>
                                    <ImageList sx={{ width: '100%', borderRadius: 4, overflow: 'hidden' }} cols={3} gap={8}>
                                        {ticket.photos.map((url: string, index: number) => (
                                            <ImageListItem key={index}><img src={url} alt={`issue-${index}`} loading="lazy" style={{ height: 150, objectFit: 'cover' }} /></ImageListItem>
                                        ))}
                                    </ImageList>
                                </Box>
                            )}
                    ...
                    {isCompleted && (
                        <Paper sx={{ p: 4, mb: 4, bgcolor: alpha('#10b981', 0.05), border: '1px solid #10b981', borderRadius: 6 }}>
                            <Typography variant="h6" fontWeight="950" color="#10b981" sx={{ mb: 2, display: 'flex', alignItems: 'center', gap: 1.5 }}>
                                <CheckCircle2 /> {tx('ticket.work_completed_title', 'WORK COMPLETED — REVIEW REQUIRED')}
                            </Typography>
                            <Typography variant="body2" color="rgba(255,255,255,0.6)" sx={{ mb: 4 }}>
                                {tx('ticket.work_completed_desc', 'Please check the work, rate the technician, and either approve the service or dispute it with a reason. Your rating will be saved on the ticket record.')}
                            </Typography>
                            {ticket.technicianNotes && (
                                <Box sx={{ mb: 4, p: 2, bgcolor: 'rgba(255,255,255,0.02)', borderRadius: 3, border: '1px solid rgba(255,255,255,0.05)' }}>
                                    <Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.3)', fontWeight: 900 }}>{tx('ticket.tech_notes_label', 'TECHNICIAN RESOLUTION NOTES')}</Typography>
                            ...
                            {(beforeProof || afterProof) && (
                                <Grid container spacing={2} sx={{ mb: 4 }}>
                                    <Grid item xs={6}>
                                        <Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.4)', fontWeight: 900, mb: 1, display: 'block' }}>{tx('ticket.before_label', 'BEFORE')}</Typography>
                                    ...
                                    <Grid item xs={6}>
                                        <Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.4)', fontWeight: 900, mb: 1, display: 'block' }}>{tx('ticket.after_label', 'AFTER')}</Typography>
                                        <Box sx={{ borderRadius: 3, overflow: 'hidden', pt: '75%', position: 'relative', bgcolor: 'rgba(0,0,0,0.3)' }}>
                            ...
                            <Box sx={{ mb: 3, p: 3, borderRadius: 4, bgcolor: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
                                <Typography variant="caption" sx={{ color: binThemeTokens.gold, fontWeight: 950, display: 'block', mb: 1 }}>{tx('ticket.rate_label', 'RATE TECHNICIAN SERVICE')}</Typography>
                                <Rating value={rating} onChange={(_, nextValue) => setRating(nextValue || 1)} size="large" sx={{ mb: 2, '& .MuiRating-iconFilled': { color: binThemeTokens.gold }, '& .MuiRating-iconEmpty': { color: 'rgba(255,255,255,0.25)' } }} />
                                <TextField fullWidth multiline rows={3} label={tx('ticket.feedback_label', 'Feedback for technician / BIN GROUP')} value={feedback} onChange={(e) => setFeedback(e.target.value)} placeholder={tx('ticket.feedback_placeholder', 'Example: Technician arrived on time and fixed the issue properly.')} sx={{ '& .MuiOutlinedInput-root': { bgcolor: 'rgba(255,255,255,0.02)', color: '#FFF' }, '& label': { color: 'rgba(255,255,255,0.5)' } }} />
                            </Box>
                            {!showRejectInput ? (
                                <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                                    <Button fullWidth variant="contained" color="success" startIcon={actionLoading ? <CircularProgress size={18} color="inherit" /> : <Check />} onClick={handleApprove} disabled={actionLoading || !rating} sx={{ fontWeight: 950, py: 1.5, borderRadius: 3 }}>{tx('ticket.approve_button', 'APPROVE, RATE & CLOSE')}</Button>
                                    <Button fullWidth variant="outlined" color="error" startIcon={<X />} onClick={() => setShowRejectInput(true)} disabled={actionLoading} sx={{ fontWeight: 950, py: 1.5, borderRadius: 3 }}>{tx('ticket.dispute_button', 'DISPUTE SERVICE')}</Button>
                                </Stack>
                            ) : (
                                <Stack spacing={2}>
                                    <TextField fullWidth multiline rows={3} label={tx('ticket.dispute_reason_label', 'Reason for disputing resolution')} value={rejectReason} onChange={(e) => setRejectReason(e.target.value)} sx={{ '& .MuiOutlinedInput-root': { bgcolor: 'rgba(255,255,255,0.02)', color: '#FFF' }, '& label': { color: 'rgba(255,255,255,0.5)' } }} />
                                    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                                        <Button fullWidth variant="contained" color="error" onClick={handleReject} disabled={actionLoading || !(rejectReason || feedback).trim()} sx={{ fontWeight: 950, borderRadius: 3 }}>{tx('ticket.confirm_dispute', 'CONFIRM DISPUTE')}</Button>
                                        <Button fullWidth variant="text" onClick={() => setShowRejectInput(false)} sx={{ color: 'rgba(255,255,255,0.5)', fontWeight: 900 }}>{tx('ticket.cancel', 'CANCEL')}</Button>
                                    </Stack>
                                </Stack>
                            )}
                        </Paper>
                    ...
                    {isDisputed && (
                        <Paper sx={{ p: 4, mb: 4, bgcolor: alpha('#ef4444', 0.05), border: '1px solid #ef4444', borderRadius: 6 }}>
                            <Typography variant="h6" fontWeight="950" color="#ef4444" sx={{ mb: 1, display: 'flex', alignItems: 'center', gap: 1.5 }}>
                                <AlertCircle /> {tx('ticket.disputed_title', 'JOB DISPUTED')}
                            </Typography>
                            <Typography variant="body2" color="rgba(255,255,255,0.6)">{tx('ticket.disputed_desc', 'You have rejected the resolution. Property Management is reviewing the case.')}</Typography>
                            <Box sx={{ mt: 3, p: 2, bgcolor: 'rgba(255,255,255,0.02)', borderRadius: 3 }}>
                                <Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.3)', fontWeight: 900 }}>{tx('ticket.your_dispute_reason', 'YOUR DISPUTE REASON')}</Typography>
                    ...
                    {isClosed && (
                        <Paper sx={{ p: 4, mb: 4, bgcolor: alpha('#10b981', 0.05), border: '1px solid rgba(16, 185, 129, 0.3)', borderRadius: 6, textAlign: 'center' }}>
                            ...
                            <Typography variant="h6" fontWeight="950" color="#10b981">{tx('ticket.closed_title', 'SERVICE FINALIZED')}</Typography>
                            <Typography variant="body2" color="rgba(255,255,255,0.4)" sx={{ mb: ticket.rating || ticket.feedback ? 2 : 0 }}>{tx('ticket.closed_desc', 'This ticket has been successfully closed and archived.')}</Typography>
                ...
                <Grid item xs={12} lg={4}>
                    <Box sx={{ mb: 3 }}>
                        <LiveTechnicianTrackingCard ticket={ticket} onChatClick={() => navigate(`/tenant/chat/${ticket.id}`)} onCallClick={() => { const phone = ticket.assignedTechnicianPhone || ticket.technicianPhone || ticket.tenantPhone; if (phone) window.open(`tel:${phone}`); }} showTimeline={true} />
                    </Box>
                    <Paper sx={{ p: 3, bgcolor: alpha(binThemeTokens.gold, 0.02), border: '1px solid rgba(255,255,255,0.03)', borderRadius: 5 }}>
                        <Typography variant="subtitle2" fontWeight="950" color={binThemeTokens.gold} sx={{ mb: 1, display: 'flex', alignItems: 'center', gap: 1 }}>
                            <Info size={16} /> {tx('ticket.assistance_title', 'NEED ASSISTANCE?')}
                        </Typography>
                        <Typography variant="caption" color="rgba(255,255,255,0.4)" sx={{ fontWeight: 700, display: 'block', minWidth: 0, wordBreak: 'break-word', overflowWrap: 'anywhere' }}>
                            {tx('ticket.assistance_desc', 'Use the chat or call button above to reach your technician directly. For escalations, contact BIN GROUP concierge.')}
                        </Typography>
                    </Paper>
                </Grid>
            </Grid>
