...
export default function TenantDashboardPage() {
  ...
  const serviceButtons = useMemo(() => [
    { label: t('service.deep_cleaning') || 'Deep Cleaning', icon: <Sparkles size={20} />, route: '/tenant/request?category=cleaning', visible: true },
    { label: t('service.moving') || 'Moving & Packing', icon: <Truck size={20} />, route: '/tenant/request?category=moving', visible: true },
    { label: showMaintenance ? (t('nav.maintenance') || 'Maintenance') : 'Contact Management', icon: <Wrench size={20} />, route: showMaintenance ? '/tenant/request' : '/tenant/request?category=management', visible: true },
    { label: t('nav.ai_studio') || 'AI Design Studio', icon: <Paintbrush size={20} />, route: '/tenant/design-studio', visible: true },
    { label: 'Gate Pass', icon: <ShieldCheck size={20} />, route: '/tenant/gate-pass', visible: showManagement },
    { label: 'Amenities', icon: <Dumbbell size={20} />, route: '/tenant/amenities', visible: showManagement },
  ].filter((button) => button.visible), [showMaintenance, showManagement, t]);
      ...
      <Stack spacing={4}>
        ...
        <Paper sx={{ p: { xs: 3, md: 4 }, bgcolor: 'rgba(15,23,42,0.72)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 6 }}>
          ...
          <Grid container spacing={2}>
            <Grid item xs={12} md={3}><Box sx={{ p: 2, bgcolor: 'rgba(255,255,255,0.03)', borderRadius: 3 }}><Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.42)', fontWeight: 950 }}>PROPERTY TYPE</Typography><Typography sx={{ color: '#fff', fontWeight: 950 }}>{propertyProfile.propertyClass.replace(/_/g, ' ')}</Typography></Box></Grid>
            <Grid item xs={12} md={3}><Box sx={{ p: 2, bgcolor: 'rgba(255,255,255,0.03)', borderRadius: 3 }}><Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.42)', fontWeight: 950 }}>LEASE / RENT</Typography><Typography sx={{ color: showLedger ? '#10b981' : '#f59e0b', fontWeight: 950 }}>{showLedger ? 'Visible' : 'Not Included'}</Typography></Box></Grid>
            <Grid item xs={12} md={3}><Box sx={{ p: 2, bgcolor: 'rgba(255,255,255,0.03)', borderRadius: 3 }}><Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.42)', fontWeight: 950 }}>MAINTENANCE</Typography><Typography sx={{ color: showMaintenance ? '#10b981' : '#f59e0b', fontWeight: 950 }}>{showMaintenance ? 'Enabled' : 'Management Routed'}</Typography></Box></Grid>
            <Grid item xs={12} md={3}><Box sx={{ p: 2, bgcolor: 'rgba(255,255,255,0.03)', borderRadius: 3 }}><Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.42)', fontWeight: 950 }}>UNIT / AREA</Typography><Typography sx={{ color: '#fff', fontWeight: 950 }}>{unitData?.unitNumber || unitData?.unitId || 'Property Access'}</Typography></Box></Grid>
          </Grid>
        </Paper>
