...
export default function TenantMoveInspectionPage() {
  ...
  return (
    <Container maxWidth="md" sx={{ py: 4, direction: isRTL ? 'rtl' : 'ltr' }}>
      <Paper sx={{ p: { xs: 3, md: 4 }, bgcolor: 'rgba(15,23,42,.88)', border: `1px solid ${alpha(binThemeTokens.gold, 0.2)}`, borderRadius: '24px 24px 0 0' }}>
        <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ xs: 'flex-start', sm: 'center' }} spacing={2}>
          <Stack direction="row" spacing={2} alignItems="center">
            <IconButton aria-label="Back to tenant move inspections" onClick={() => navigate(-1)} sx={{ color: 'rgba(255,255,255,0.5)', transform: isRTL ? 'rotate(180deg)' : 'none' }}><ChevronLeft /></IconButton>
            <Box>
      ...
      <Dialog open={!!previewPhoto} onClose={() => setPreviewPhoto(null)} maxWidth="md" PaperProps={{ sx: { bgcolor: '#020617', borderRadius: 4 } }}><Box sx={{ position: 'relative' }}><IconButton aria-label="Close inspection photo preview" onClick={() => setPreviewPhoto(null)} sx={{ position: 'absolute', top: 8, right: 8, bgcolor: 'rgba(0,0,0,0.6)', color: '#fff', zIndex: 1 }}><X size={18} /></IconButton>{previewPhoto && <Box component="img" src={previewPhoto} sx={{ maxWidth: '100%', maxHeight: '80vh', display: 'block' }} />}</Box></Dialog>
