...
export default function BrokerAttributionProofPage() {
  ...
  useEffect(() => {
    ...
    unsubs.push(bind('brokerLeads', setLeads));
    unsubs.push(bind('referrals', setReferrals));
    unsubs.push(bind('broker_commissions', setCommissions));
        ...
        <Grid container spacing={2}>
          {[{ label: 'Chains', value: chains.length, icon: <Link2 />, color: gold }, { label: 'Locked / paid', value: locked, icon: <ShieldCheck />, color: '#10b981' }, { label: 'With commission', value: withCommission, icon: <Wallet />, color: '#3b82f6' }, { label: 'Missing links', value: missingLead + missingReferral, icon: <FileText />, color: missingLead + missingReferral ? '#f59e0b' : '#10b981' }].map((card) => (
            <Grid item xs={12} sm={6} md={3} key={card.label}>
              <Paper sx={{ p: 3, borderRadius: 4, border: `1px solid ${alpha(card.color, 0.24)}`, bgcolor: alpha(card.color, 0.06) }}>
                <Box sx={{ color: card.color, mb: 1 }}>{card.icon}</Box>
                <Typography variant="h4" sx={{ color: '#111827', fontWeight: 950 }}>{card.value}</Typography>
                <Typography variant="caption" sx={{ color: '#667085', fontWeight: 900 }}>{card.label.toUpperCase()}</Typography>
              </Paper>
            </Grid>
          ))}
        </Grid>
              ...
              <Paper key={chain.id} sx={{ p: 3, borderRadius: 5, border: `1px solid ${alpha(commissionColor, 0.22)}`, bgcolor: '#fff' }}>
                <Grid container spacing={2} alignItems="stretch">
                  <Grid item xs={12} md={4}>
                    <Stack spacing={1}>
                      <Typography variant="overline" sx={{ color: gold, fontWeight: 950 }}>1. Lead Source</Typography>
                      <Typography sx={{ color: '#111827', fontWeight: 950 }}>{chain.lead?.clientName || chain.lead?.ownerName || chain.referral?.clientName || 'Lead not linked'}</Typography>
                      <Typography variant="caption" sx={{ color: '#667085', fontWeight: 800 }}>{chain.lead?.phone || chain.lead?.email || chain.referral?.clientEmail || 'Contact pending'}</Typography>
                      <Chip size="small" label={chain.lead ? 'LEAD LINKED' : 'MISSING LEAD'} sx={{ width: 'fit-content', bgcolor: alpha(chain.lead ? '#10b981' : '#f59e0b', 0.12), color: chain.lead ? '#10b981' : '#f59e0b', fontWeight: 950 }} />
                    </Stack>
                  </Grid>
                  <Grid item xs={12} md={4}>
                    <Stack spacing={1}>
                      <Typography variant="overline" sx={{ color: gold, fontWeight: 950 }}>2. Referral / Contract</Typography>
                      <Typography sx={{ color: '#111827', fontWeight: 950 }}>{chain.referral?.propertyName || chain.referral?.propertyId || commission?.propertyName || 'Property pending'}</Typography>
                      <Typography variant="caption" sx={{ color: '#667085', fontWeight: 800 }}>Contract: {chain.referral?.contractId || commission?.contractId || 'Not linked yet'}</Typography>
                      <Chip size="small" label={chain.referral ? String(chain.referral.status || 'SUBMITTED').replace(/_/g, ' ').toUpperCase() : 'NO REFERRAL'} sx={{ width: 'fit-content', bgcolor: alpha(statusColor(chain.referral?.status), 0.12), color: statusColor(chain.referral?.status), fontWeight: 950 }} />
                    </Stack>
                  </Grid>
                  <Grid item xs={12} md={4}>
                    <Stack spacing={1}>
                      <Typography variant="overline" sx={{ color: gold, fontWeight: 950 }}>3. Commission Proof</Typography>
                      <Typography sx={{ color: '#111827', fontWeight: 950 }}>{money(commission?.amount || commission?.commissionAmount)}</Typography>
                      <Typography variant="caption" sx={{ color: '#667085', fontWeight: 800 }}>Source: {commission?.sourceContractId || commission?.contractId || chain.referral?.contractId || 'Pending contract source'}</Typography>
                      <Chip size="small" icon={(commission?.attributionLocked || String(commission?.status || '').toUpperCase().includes('PAID')) ? <CheckCircle2 size={14} /> : undefined} label={commission ? String(commission.status || (commission.attributionLocked ? 'LOCKED' : 'PENDING')).replace(/_/g, ' ').toUpperCase() : 'NO COMMISSION'} sx={{ width: 'fit-content', bgcolor: alpha(commissionColor, 0.12), color: commissionColor, fontWeight: 950 }} />
                    </Stack>
                  </Grid>
                </Grid>
                <Divider sx={{ my: 2 }} />
                <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
                  <Chip size="small" label={`Referral ID: ${chain.referral?.id || 'missing'}`} />
                  <Chip size="small" label={`Lead ID: ${chain.lead?.id || chain.referral?.leadId || 'missing'}`} />
                  <Chip size="small" label={`Commission IDs: ${chain.commissions.map((item) => item.id).join(', ') || 'missing'}`} />
                  <Button endIcon={<ArrowRight size={14} />} onClick={() => navigate('/broker/commissions')} sx={{ ml: 'auto', color: gold, fontWeight: 950 }}>Open commissions</Button>
                </Stack>
              </Paper>
            );
          })}
          {chains.length === 0 && (
            <Paper sx={{ p: 6, textAlign: 'center', borderRadius: 5, border: `1px dashed ${alpha(gold, 0.28)}` }}>
              <Briefcase size={44} color={gold} style={{ margin: '0 auto 12px' }} />
              <Typography sx={{ color: '#111827', fontWeight: 950 }}>No attribution chains yet</Typography>
              <Typography sx={{ color: '#667085', mt: 1 }}>Submit leads and referrals to build commission proof chains.</Typography>
            </Paper>
          )}
        </Stack>
      </Stack>
