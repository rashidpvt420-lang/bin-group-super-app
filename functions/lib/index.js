...
exports.onIntakeCreated = exports.onMaintenanceTicketCreated = exports.financialSettlementEngine = exports.paymentWebhook = exports.createPaymentIntent = exports.checkAssetHealthPredictive = exports.checkGovernmentCertifications = exports.checkHotelCompliance = exports.checkSchoolCompliance = exports.onTicketResolved = exports.checkContractRenewals = exports.onTicketEscalation = exports.onContractActivated = exports.onTicketCreated = exports.evaluateSLACron = exports.setAdminRole = exports.autoGrantAdminOnFirstLogin = exports.verifyAppCheck = void 0;
 ...
 */
const verifyAppCheck = async (req, consume = false) => {
    const appCheckToken = req.header("X-Firebase-AppCheck");
    if (!appCheckToken) {
        throw new https_1.HttpsError("unauthenticated", "Request is missing a valid App Check token.");
    }
    try {
        ...
        if (consume && appCheckClaims.alreadyConsumed) {
            throw new https_1.HttpsError("aborted", "This request has already been processed (Replay detected).");
        }
        return appCheckClaims;
    }
    catch (err) {
        console.error("[SECURITY] App Check Failure:", err);
        throw new https_1.HttpsError("unauthenticated", "Unauthorized app access attempt.");
    }
};
...
exports.autoGrantAdminOnFirstLogin = (0, identity_1.beforeUserCreated)(async (event) => {
 ...
 */
exports.setAdminRole = (0, https_1.onCall)({ enforceAppCheck: true }, async (request) => {
    const caller = request.auth;
    if (!caller?.token?.godMode) {
        throw new https_1.HttpsError("permission-denied", "Only God-Mode accounts can promote users.");
    }
    const { email, role, isAdmin, godMode: grantGodMode } = request.data;
    if (!email)
...
exports.evaluateSLACron = (0, scheduler_1.onSchedule)("every 4 hours", async (event) => {
    ...
    staleTickets.forEach((doc) => {
        ...
        const contractTier = ticketData.contractTier || "BASIC";
...
exports.onTicketCreated = (0, firestore_1.onDocumentCreated)("tickets/{ticketId}", async (event) => {
...
exports.onContractActivated = (0, firestore_1.onDocumentUpdated)("active_contracts/{contractId}", async (event) => {
    if (!event.data)
        return;
...
exports.onTicketEscalation = (0, firestore_1.onDocumentUpdated)("tickets/{ticketId}", async (event) => {
    if (!event.data)
        return;
...
exports.checkContractRenewals = (0, scheduler_1.onSchedule)("every 24 hours", async (event) => {
    const now = admin.firestore.Timestamp.now();
...
exports.onTicketResolved = (0, firestore_1.onDocumentUpdated)("tickets/{ticketId}", async (event) => {
    if (!event.data)
        return;
    const newData = event.data.after.data();
    const oldData = event.data.before.data();
...
exports.checkSchoolCompliance = (0, scheduler_1.onSchedule)("every 24 hours", async (event) => {
    const schools = await db.collection("active_contracts").where("propertyType", "==", "School").get();
    const now = admin.firestore.Timestamp.now();
    schools.forEach(async (doc) => {
        const data = doc.data();
        const alerts = [];
        if (!data.lastFireAlarmInspection)
            alerts.push("Fire Alarm Due");
        if (!data.lastWaterTankCleaning)
            alerts.push("Water Tank Sanitization Due");
        if (alerts.length > 0) {
            await db.collection("notifications").add({
                userId: data.ownerId,
...
exports.checkHotelCompliance = (0, scheduler_1.onSchedule)("every 24 hours", async (event) => {
    const hotels = await db.collection("active_contracts").where("propertyType", "==", "Hotel").get();
    const now = admin.firestore.Timestamp.now();
    hotels.forEach(async (doc) => {
        const data = doc.data();
...
exports.checkGovernmentCertifications = (0, scheduler_1.onSchedule)("every 24 hours", async (event) => {
...
exports.checkAssetHealthPredictive = (0, scheduler_1.onSchedule)("every 24 hours", async (event) => {
 ...
 */
exports.createPaymentIntent = (0, https_1.onCall)({ enforceAppCheck: true }, async (request) => {
    const { amount, currency, propertySnapshot, planSnapshot, addOnsSnapshot } = request.data;
    if (!amount || !propertySnapshot || !planSnapshot) {
        throw new https_1.HttpsError("invalid-argument", "Missing protocol parameters for Asset Handshake.");
    }
 ...
 */
exports.financialSettlementEngine = (0, https_1.onCall)({ enforceAppCheck: true }, async (request) => {
    const caller = request.auth;
    // Authorization Check
    if (!caller?.token?.admin) {
        throw new https_1.HttpsError("permission-denied", "Only BIN-ADMIN or higher can trigger sovereign settlement.");
    }
    const { action, paymentId, gross_amount, ownerId, propertyId, type } = request.data;
    if (!paymentId || !ownerId) {
        throw new https_1.HttpsError("invalid-argument", "Missing financial identification parameters.");
    }
    ...
    throw new https_1.HttpsError("invalid-argument", "Unsupported settlement action.");
});
...
exports.onMaintenanceTicketCreated = (0, firestore_1.onDocumentCreated)("maintenanceTickets/{ticketId}", async (event) => {
    const snap = event.data;
    if (!snap)
        return;
    const ticket = snap.data();
 ...
 */
exports.onIntakeCreated = (0, firestore_1.onDocumentCreated)("intake_submissions/{intakeId}", async (event) => {
    const snap = event.data;
    if (!snap)
        return;
    ...
    const hasSovereign = properties.some((p) => p.assetGrade === 'Sovereign' || p.majlisType === 'sovereign');
    const isCommercial = properties.some((p) => p.propertyType === 'Commercial' || p.propertyType === 'Institutional');
    ...
    if (hasSovereign)
        maintenanceForecastArr.push({ item: "Sovereign Special Protocol", value: 25000, period: "annual" });
    const assessment = {
