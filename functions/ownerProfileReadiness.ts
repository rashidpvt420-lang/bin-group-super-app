...
async function ownedDocuments(collectionName: string, uid: string) {
  ...
  for (const snapshot of snapshots) {
    for (const item of snapshot.docs) deduplicated.set(item.id, { id: item.id, ...item.data() });
  }
...
export const getOwnerProfileReadiness = onCall(
  ...
  async (request) => {
    ...
    const [userSnap, properties, contracts, payments, intakes] = await Promise.all([
      ...
      ownedDocuments("owner_onboarding_intakes", uid),
    ]);
    const profile = userSnap.data() || {};
    const identityStatus = profile.kycStatus || profile.identityStatus || profile.verificationStatus || profile.ownerKycStatus;
    const identityVerified = profile.kycVerified === true || profile.identityVerified === true || profile.ownerVerified === true || approvedStatus(identityStatus);
    const phoneVerified = Boolean(authRecord.phoneNumber) && (profile.phoneVerified === true || profile.phoneAuthority === "FIREBASE_AUTH_PHONE");
    const propertyProofApproved = properties.some((item) => approvedStatus(item.titleDeedStatus || item.propertyProofStatus || item.verificationStatus)) || intakes.some((item) => approvedStatus(item.documentStatus || item.kycStatus || item.verificationStatus));
    const locationApproved = properties.some((item) => item.geo?.verified === true && item.geo?.dispatchReady === true) || intakes.some((item) => item.property?.geo?.verified === true && item.property?.geo?.dispatchReady === true);
    const contractSigned = contracts.some((item) => item.signedAt || signedStatus(item.status || item.contractStatus)) || intakes.some((item) => item.signatureName && item.otpVerificationId);
