...
export const requestBrokerPayoutOtp = onCall({ cors: true, region: "europe-west3", enforceAppCheck: true, secrets: [smtpUser, smtpPass] }, async (request) => {
  ...
  await ref.set({ uid: broker.uid, email: broker.email, commissionIds: commissions.ids, amount: commissions.amount, currency: "AED", bindingHash, otpHash: hashOtp(otp, salt), salt, attempts: 0, status: "PENDING", expiresAt, delivery: { messageId, sentAt: FieldValue.serverTimestamp() }, createdAt: FieldValue.serverTimestamp(), updatedAt: FieldValue.serverTimestamp() });
  await db.collection("audit_logs").add({ action: "BROKER_PAYOUT_OTP_SENT", actorId: broker.uid, challengeId: ref.id, bindingHash, createdAt: FieldValue.serverTimestamp() });
...
export const verifyBrokerPayoutOtp = onCall({ cors: true, region: "europe-west3", enforceAppCheck: true }, async (request) => {
  ...
  const messages: Record<string, [any, string]> = {
    NOT_FOUND: ["not-found", "Payout OTP challenge not found."], FORBIDDEN: ["permission-denied", "Payout OTP challenge belongs to another Broker."], CONSUMED: ["failed-precondition", "Payout OTP evidence was already consumed."], EXPIRED: ["deadline-exceeded", "Payout OTP expired."], MAX_ATTEMPTS: ["resource-exhausted", "Maximum OTP attempts exceeded."], INVALID: ["permission-denied", "Invalid payout OTP."],
  };
...
export const submitBrokerPayoutRequest = onCall({ cors: true, region: "europe-west3", enforceAppCheck: true }, async (request) => {
  ...
  await db.runTransaction(async (tx) => {
    ...
    tx.set(payoutRef, { brokerId: broker.uid, brokerUid: broker.uid, brokerEmail: broker.email, brokerName: text(broker.profile.displayName || broker.profile.name) || "Broker", brokerCode: text(broker.profile.brokerCode || broker.profile.affiliateCode) || `BIN-${broker.uid.slice(0, 8).toUpperCase()}`, amount: commissions.amount, currency: "AED", commissionIds: commissions.ids, commissionCount: commissions.ids.length, bankName: text(broker.profile.bankName), bankAccountHolder: text(broker.profile.bankAccountHolder || broker.profile.displayName || broker.profile.name), bankIban: text(broker.profile.bankIban || broker.profile.iban), status: "PENDING_ADMIN_REVIEW", approvalStatus: "PENDING", paymentStatus: "REQUESTED", verificationState: "EMAIL_OTP_SINGLE_USE", mfaChallengeId: challengeId, mfaBindingHash: bindingHash, requestedBy: broker.uid, requestedAt: now, createdAt: now, updatedAt: now });
    ...
    tx.set(db.collection("audit_logs").doc(), { action: "BROKER_PAYOUT_REQUEST_SUBMITTED_WITH_OTP", actorId: broker.uid, payoutRequestId: payoutRef.id, challengeId, bindingHash, commissionIds: commissions.ids, amount: commissions.amount, createdAt: now });
  });
