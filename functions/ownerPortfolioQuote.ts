...
function assertVerifiedOwner(request: any): string {
  ...
  if (roleOf(request.auth) !== "owner") {
    throw new HttpsError("permission-denied", "Only an owner can issue a portfolio quote.");
  }
...
function cleanPropertyId(value: unknown): string {
  ...
  if (!/^[A-Za-z0-9_-]{1,120}$/.test(id)) {
    throw new HttpsError("invalid-argument", "Every property requires a valid property ID.");
  }
  return id;
}
...
function calculatePortfolio(properties: QuotePropertyRequest[]): {
  ...
  const propertyQuotes = properties.map(({ id, input }) => ({
    propertyId: id,
    input,
    output: calculateUaeQuote2026(input),
  }));
  ...
  if (portfolioAnnualTotal <= 0) {
    throw new HttpsError("failed-precondition", "The server quote produced no payable annual value.");
  }
...
export const issueOwnerPortfolioQuote = onCall(
  ...
  async (request) => {
    ...
    const inputEnvelope = {
      ownerUid,
      quoteSchemaVersion: QUOTE_SCHEMA_VERSION,
    ...
    await db.collection("audit_logs").add({
      action: "OWNER_PORTFOLIO_QUOTE_ISSUED",
      actorId: ownerUid,
      actorRole: "owner",
      quoteId,
      quoteSchemaVersion: QUOTE_SCHEMA_VERSION,
      ...
      portfolioAnnualTotal: calculated.portfolioAnnualTotal,
      mobilisationDeposit: calculated.mobilisationDeposit,
      expiresAtMs,
      createdAt: admin.firestore.FieldValue.serverTimestamp(),
    });
...
export const validateOwnerPortfolioQuote = onCall(
  ...
  async (request) => {
    const ownerUid = assertVerifiedOwner(request);
    const quoteId = cleanPropertyId(request.data?.quoteId);
    const suppliedQuoteHash = String(request.data?.quoteHash || "").trim().toLowerCase();
    const suppliedInputHash = String(request.data?.inputHash || "").trim().toLowerCase();
    ...
    if (Number(quote.expiresAtMs || 0) <= Date.now()) {
      throw new HttpsError("failed-precondition", "The quote expired. Generate a new server quote.");
    }
    ...
    const expectedAnnualTotal = request.data?.portfolioAnnualTotal;
    const expectedDeposit = request.data?.mobilisationDeposit;
    if (expectedAnnualTotal !== undefined && money(expectedAnnualTotal, "Portfolio annual total") !== Number(quote.portfolioAnnualTotal)) {
      throw new HttpsError("failed-precondition", "Portfolio annual total does not match the server quote.");
    }
    if (expectedDeposit !== undefined && money(expectedDeposit, "Mobilisation deposit") !== Number(quote.mobilisationDeposit)) {
    ...
    return {
      ...
      portfolioAnnualTotal: quote.portfolioAnnualTotal,
      mobilisationDeposit: quote.mobilisationDeposit,
      currency: "AED",
    };
  },
);
