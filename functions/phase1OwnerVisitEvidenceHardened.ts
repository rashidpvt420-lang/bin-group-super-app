...
const MAX_ARRIVAL_DISTANCE_METRES = 250;
const MAX_ARRIVAL_ACCURACY_METRES = 100;
const MAX_GPS_CAPTURE_AGE_MS = 2 * 60 * 1000;
...
function inspectionLocation(value: PlainRecord) {
  ...
  if (!Number.isFinite(lat) || !Number.isFinite(lng)) throw new HttpsError("failed-precondition", "The inspection does not have a valid submitted property location.");
  return { lat, lng };
}

function assertCapturedBrowserGps(data: PlainRecord, startedAtMs: number, completedAtMs: number) {
  const lat = coordinate(data.arrivalLat, -90, 90, "Arrival latitude");
  const lng = coordinate(data.arrivalLng, -180, 180, "Arrival longitude");
  const accuracyMetres = finite(data.arrivalAccuracyMetres, Number.NaN);
  const capturedAtMs = finite(data.positionCapturedAtMs);
  const nowMs = Date.now();
  if (!Number.isFinite(accuracyMetres) || accuracyMetres <= 0 || accuracyMetres > MAX_ARRIVAL_ACCURACY_METRES) {
    throw new HttpsError("failed-precondition", `Current browser GPS accuracy must be ${MAX_ARRIVAL_ACCURACY_METRES} metres or better.`);
  }
  if (
    !capturedAtMs ||
    capturedAtMs < nowMs - MAX_GPS_CAPTURE_AGE_MS ||
    capturedAtMs > nowMs + 60_000 ||
    capturedAtMs < startedAtMs - 60_000 ||
    capturedAtMs > completedAtMs + 60_000
  ) {
    throw new HttpsError("failed-precondition", "Capture fresh device GPS during the recorded site visit before submitting evidence.");
  }
  return { lat, lng, accuracyMetres, capturedAtMs };
}

export const adminRecordOwnerPortfolioVisitEvidenceHardened = onCall(
  ...
  async (request) => {
    ...
    const intakeId = text(data.intakeId);
    const inspectionId = text(data.inspectionId);
    const findings = text(data.findings).slice(0, 4000);
    ...
    const startedAtMs = finite(data.startedAtMs);
    const completedAtMs = finite(data.completedAtMs);
    const nowMs = Date.now();
    ...
    const capturedGps = assertCapturedBrowserGps(data, startedAtMs, completedAtMs);
    const checklist = requiredChecklist(data.checklist);

    const encodedPhoto = text(data.encodedPhoto);
    const filename = text(data.filename || "property-visit.jpg").replace(/[^A-Za-z0-9._-]/g, "_").slice(0, 160);
    const contentType = lower(data.contentType || "image/jpeg");
    ...
    const arrivalDistanceMetres = Math.round(distanceMetres(capturedGps.lat, capturedGps.lng, expectedLocation.lat, expectedLocation.lng));
    if (arrivalDistanceMetres > MAX_ARRIVAL_DISTANCE_METRES) {
      throw new HttpsError("failed-precondition", `Current device GPS is ${arrivalDistanceMetres} metres from the submitted property. Move within ${MAX_ARRIVAL_DISTANCE_METRES} metres.`);
    }
    ...
    await file.save(photoBuffer, {
      ...
      metadata: {
        ...
        metadata: {
          ...
          gpsCaptureSource: "BROWSER_GEOLOCATION_AT_SUBMISSION",
          gpsAccuracyMetres: String(Math.round(capturedGps.accuracyMetres)),
          gpsCapturedAtMs: String(capturedGps.capturedAtMs),
    ...
    const visitEvidence = {
      schemaVersion: 2,
      captureSource: "BROWSER_GEOLOCATION_AT_SUBMISSION",
      arrival: {
        lat: capturedGps.lat,
        lng: capturedGps.lng,
        point: new admin.firestore.GeoPoint(capturedGps.lat, capturedGps.lng),
        accuracyMetres: capturedGps.accuracyMetres,
        capturedAt: admin.firestore.Timestamp.fromMillis(capturedGps.capturedAtMs),
      ...
      maximumAllowedAccuracyMetres: MAX_ARRIVAL_ACCURACY_METRES,
      gpsWithinRadius: true,
      gpsAccuracyVerified: true,
      gpsCaptureFresh: true,
    ...
    batch.set(db.collection("audit_logs").doc(), {
      ...
      metadata: {
        ...
        propertyId,
        arrivalDistanceMetres,
        gpsAccuracyMetres: capturedGps.accuracyMetres,
        gpsCapturedAtMs: capturedGps.capturedAtMs,
        sha256,
        generation,
        durationMs,
      },
...
export const adminCompleteOwnerPortfolioInspectionsPhase1Hardened = onCall(
  ...
  async (request) => {
    ...
    inspectionSnaps.forEach((snapshot, index) => {
      ...
      const arrivalAccuracy = finite(evidence.arrival?.accuracyMetres, Number.NaN);
      const arrivalDistance = finite(evidence.arrivalDistanceMetres, Number.NaN);
      if (
        text(value.intakeId) !== intakeId ||
        upper(value.status) === "CANCELLED" ||
        value.evidenceVerified !== true ||
        evidence.captureSource !== "BROWSER_GEOLOCATION_AT_SUBMISSION" ||
        evidence.gpsWithinRadius !== true ||
        evidence.gpsAccuracyVerified !== true ||
        evidence.gpsCaptureFresh !== true ||
        !Number.isFinite(arrivalDistance) || arrivalDistance > MAX_ARRIVAL_DISTANCE_METRES ||
        !Number.isFinite(arrivalAccuracy) || arrivalAccuracy > MAX_ARRIVAL_ACCURACY_METRES ||
        evidence.checklistComplete !== true ||
        finite(evidence.photoCount) < 1 ||
        text(evidence.findings).length < 8 ||
        !evidence.startedAt ||
        !evidence.completedAt ||
        !evidence.arrival?.capturedAt ||
        !propertyId
      ) throw new HttpsError("failed-precondition", `Inspection ${index + 1} requires fresh accurate browser GPS, checklist, findings, timestamps and photo evidence.`);
    ...
    batch.set(intakeRef, {
      ...
      locationVerificationStatus: "VERIFIED_ADMIN_SITE_GPS",
    ...
    propertyQuery.docs.forEach((document) => {
      ...
      const evidence = inspection.data()?.visitEvidence || {};
      const arrival = evidence.arrival || {};
      const verifiedLat = coordinate(arrival.lat, -90, 90, "Verified property latitude");
      const verifiedLng = coordinate(arrival.lng, -180, 180, "Verified property longitude");
      const accuracyMetres = finite(arrival.accuracyMetres, Number.NaN);
      batch.set(document.ref, {
        ...
        geo: {
          ...(property.geo || {}),
          point: new admin.firestore.GeoPoint(verifiedLat, verifiedLng),
          lat: verifiedLat,
          lng: verifiedLng,
          verified: true,
          requiresGeoReview: false,
          dispatchReady: true,
          verificationSource: "ADMIN_BROWSER_GPS_SITE_VISIT",
          verificationEvidenceId: inspection.id,
          accuracyMetres,
    ...
    return {
      ...
      locationVerificationStatus: "VERIFIED_ADMIN_SITE_GPS",
      nextState: "AWAITING_PHASE1_15_PERCENT_PAYMENT",
    };
  },
);
