...
test('owner and tenant business proofs are mandatory and backend-verified', () => {
  ...
  assert.match(tenant, /Tenant → Technician → Tenant approval closes one uninterrupted real ticket/);
  assert.match(tenant, /createAndComplete\(browser, page, 'approval'\)/);
  ...
  assert.match(tenant, /verificationCodeHash/);
  assert.match(tenant, /expect\(requestData\.verificationCode\)\.toBeUndefined\(\)/);
