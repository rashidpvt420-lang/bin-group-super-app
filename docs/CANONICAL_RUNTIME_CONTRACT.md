...
`propertyGeoAuthority.ts` currently permits Founder MFA verification of a submitted coordinate and `adminPropertyReview.ts` calls it during an administrative property review. This does **not** establish physical inspection. A document review must never itself set `dispatchReady=true`, `verified=true` or `ACTIVE`. A physical inspection must be linked to each submitted property and field GPS/evidence hash/generation, timestamps and checklists validated on the server. Legacy approved property geo verification needs its own migration review, independent of the inspection-first transition.
...
| `adminPropertyReview.ts` and `propertyGeoAuthority.ts` | UNSAFE / MUST REMOVE OR ISOLATE for inspection-first property operational geo promotion | `DRAFT` review and document approval to dispatch-ready geo contradict required field evidence. Existing production data migrations need explicit review. |
