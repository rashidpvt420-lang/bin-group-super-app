...
| Candidate PR | [#1343](https://github.com/rashidpvt420-lang/bin-group-super-app/pull/1343) initially `640a406dce905cf71845082a34c1607eb31dcd4f`; **further local changes not published or deployed** | PR Validation run 35822922892 failed quote guard; the candidate SHA changes with any new commit. |
...
| P1 | Admin audit reports 23 high findings including direct `axios@1.17.0` and transitive `react-scripts@5.0.1` build dependencies (PostCSS, SVGR, CSS tooling). | Axios patched within major version in progress. CRA toolchain requires a reviewed migration; never use `npm audit fix --force` to replace `react-scripts` with the suggested `0.0.0`. | Admin audit: 57 total (10 low, 24 moderate, 23 high). Distinguish build-time tooling from shipped runtime bundle in reachability review; unresolved for public release. |
...
| `npm run build`, `npm run build:admin`, `npm run build:functions`, `npm run test:mobile-store-readiness` | PASS after memo and Functions dependency repairs; Admin rebuild after Axios pending | Main Vite, Admin CRACO, Functions TS and mobile readiness passed. Remaining Admin source-map warning from `stylis-plugin-rtl`. |
