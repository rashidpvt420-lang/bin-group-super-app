    ...
    return (
        <Container maxWidth="xl" sx={{ py: 6 }}>
            ...
            <Box sx={{ mb: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'relative', zIndex: 1 }}>
                <Box>
                    <Typography variant="h3" fontWeight="900" sx={{ color: '#FFFFFF', letterSpacing: -1 }}>Broker Ledger</Typography>
                    <Typography variant="h6" sx={{ color: binThemeTokens.textSecondary, fontWeight: 500 }}>Sovereign asset distribution and high-yield inventory.</Typography>
                </Box>
                <Button variant="contained" sx={{ background: binThemeTokens.gold, color: '#0B0B0C', fontWeight: 900, borderRadius: 3, px: 4 }}>+ LIST NEW ASSET</Button>
            </Box>

            {/* Market Intelligence */}
            <Grid container spacing={4} sx={{ mb: 8, position: 'relative', zIndex: 1 }}>
                {[
                    { label: 'TOTAL INVENTORY', val: '42 Assets', trend: 'STABLE', icon: <Building size={24} /> },
                    { label: 'ACTIVE LEADS', val: '156', trend: 'HIGH', icon: <Users size={24} /> },
                    { label: 'MARKET APPRECIATION', val: '8.2%', trend: '+1.2%', icon: <TrendingUp size={24} /> },
                    { label: 'COMMISSION INDEX', val: 'AED 450k', trend: 'READY', icon: <Landmark size={24} /> },
                ].map((kpi, i) => (
            ...
            {/* Inventory Deck */}
            <Typography variant="h5" sx={{ mb: 4, fontWeight: 900, color: '#FFFFFF', letterSpacing: 1, position: 'relative', zIndex: 1 }}>PREMIUM LISTINGS</Typography>
                        ...
                        <Paper sx={{ overflow: 'hidden', borderRadius: 6, bgcolor: 'rgba(22, 22, 24, 0.4)', border: '1px solid rgba(255,255,255,0.05)', '&:hover': { border: '1px solid rgba(198,167,94,0.3)' } }}>
                            <Box sx={{ height: 200, bgcolor: 'rgba(255,255,255,0.05)', position: 'relative' }}>
                                <Box sx={{ position: 'absolute', top: 20, right: 20 }}>
                                    <Chip label={item.yield + " YIELD"} size="small" sx={{ bgcolor: binThemeTokens.gold, color: '#0B0B0C', fontWeight: 900 }} />
                                </Box>
                                <Box 
                                    component="img" 
                                    src={item.image} 
                            ...
                            <Box sx={{ p: 4 }}>
                                ...
                                <Typography variant="body2" sx={{ color: binThemeTokens.textSecondary, mb: 3 }}>{item.loc} • SOVEREIGN UNIT</Typography>
                                ...
                                <Stack direction="row" justifyContent="space-between" alignItems="center">
                                    ...
                                    <Stack direction="row" spacing={1}>
                                        ...
                                        <Button size="small" variant="outlined" sx={{ borderColor: 'rgba(198,167,94,0.3)', color: binThemeTokens.gold, fontWeight: 900 }}>DETAILS</Button>
                                    </Stack>
                                </Stack>
                            </Box>
                        </Paper>
