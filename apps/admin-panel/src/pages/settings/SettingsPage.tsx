  ...
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      <Grid container spacing={3} sx={{ mb: 3 }}>
        ...
        <Grid item xs={12} md={3}>
          <Card sx={{ height: '100%' }}><CardContent>
            ...
            <Typography variant="body2" color="text.secondary">Approved: {methods.length ? methods.join(' + ') : 'Not recorded'}</Typography>
        ...
        <Grid item xs={12} md={3}>
          <Card sx={{ height: '100%' }}><CardContent>
            ...
            <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 1 }}>Server callable also enforces App Check.</Typography>
          </CardContent></Card>
        </Grid>
        <Grid item xs={12} md={3}>
          <Card sx={{ height: '100%' }}><CardContent>
            <Typography variant="overline" color="text.secondary">Evidence sync</Typography>
            <Typography variant="h6" sx={{ fontWeight: 900 }}>{lastSync ? lastSync.toLocaleString() : 'Waiting for protected snapshot'}</Typography>
            <Typography variant="body2" color="text.secondary">Source: {summary.sourceDocument || 'system_health/admin_summaries'}</Typography>
          </CardContent></Card>
        </Grid>
      </Grid>
              ...
              return (
                <Grid item xs={12} md={6} key={gate.key}>
                  <Card variant="outlined" sx={{ height: '100%' }}><CardContent>
                    ...
                    <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 1 }}>Verified: {readableTimestamp(evidence.verifiedAt)}</Typography>
      ...
      <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
        ...
        <Button variant="outlined" onClick={() => void loadEvidence(true)} disabled={refreshing}>{refreshing ? <CircularProgress size={18} /> : 'Refresh Evidence'}</Button>
