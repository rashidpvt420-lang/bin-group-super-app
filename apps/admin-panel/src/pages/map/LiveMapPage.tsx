...
export default function LiveMapPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribeProperties = onSnapshot(propertyQuery, (snapshot) => {
      ...
      setProperties([]);
      setPropertiesError('Canonical property verification data could not be loaded. Ticket coordinates will not be presented as verified.');
    });
  ...
  const propertiesById = useMemo(
    () => new Map(properties.map((property) => [String(property.id || property.propertyId), property])),
    [properties],
  );
  ...
  const verifiedTicketPins = useMemo(
    () => tickets
      .map((ticket) => ({ ticket, pin: resolveVerifiedTicketPin(ticket, propertiesById) }))
  ...
  useEffect(() => {
    ...
    for (const { ticket, pin } of verifiedTicketPins as Array<{ ticket: any; pin: NonNullable<ReturnType<typeof resolveVerifiedTicketPin>> }>) {
  ...
  }, [freshLocations, mapReady, verifiedTicketPins]);
      ...
      <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" spacing={2} sx={{ mb: 2 }}>
        <Box>
          ...
          <Typography variant="body2" sx={{ color: '#94a3b8' }}>
            Google Maps with authoritative property verification and canonical Technician GPS. Recorded or expired coordinates are never presented as verified.
          </Typography>
        </Box>
        <Stack direction="row" spacing={1} flexWrap="wrap">
          ...
          <Chip label={`${verifiedTicketPins.length} verified property pins`} sx={{ color: '#fff', bgcolor: '#1e3a8a' }} />
      ...
      <Grid container spacing={2}>
        <Grid item xs={12} lg={4}>
          <Paper sx={{ bgcolor: '#0f172a', color: '#fff', border: '1px solid rgba(255,255,255,0.08)', maxHeight: { lg: '76vh' }, overflow: 'auto' }}>
            <Box sx={{ p: 2, borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
              ...
              <Typography variant="caption" sx={{ color: '#94a3b8' }}>Server states only. No AI, provider or coordinate-verification claim is inferred.</Typography>
            </Box>
            <List disablePadding>
              {tickets.map((ticket) => {
                const pin = resolveVerifiedTicketPin(ticket, propertiesById);
                ...
                return (
                  <ListItem key={ticket.id} alignItems="flex-start" divider secondaryAction={
                    ...
                    <ListItemAvatar>
                      <Avatar sx={{ bgcolor: pin ? '#1d4ed8' : '#475569' }}><RoomIcon /></Avatar>
                    </ListItemAvatar>
                    <ListItemText
                        ...
                        <Box component="span" sx={{ display: 'block', color: '#94a3b8', pr: canAssign ? 8 : 0 }}>
                          ...
                          {pin ? (
                            <Button
                              ...
                              href={googleMapsSearchUrl(pin.point.lat, pin.point.lng)}
                              target="_blank"
                              rel="noreferrer"
                              sx={{ px: 0, mt: 0.5 }}
                            >
                            ...
                            <Typography component="span" variant="caption" sx={{ color: '#f59e0b' }}>
                              Recorded coordinate exists but is unverified; no map marker or verified-distance claim is shown.
                            </Typography>
        ...
        <Grid item xs={12} lg={8}>
          <Paper sx={{ position: 'relative', minHeight: { xs: 520, lg: '76vh' }, overflow: 'hidden', bgcolor: '#0f172a', border: '1px solid rgba(255,255,255,0.08)' }}>
            ...
            {mapReady && freshLocations.length === 0 && verifiedTicketPins.length === 0 && (
              <Alert severity="warning" sx={{ position: 'absolute', bottom: 16, left: 16, right: 16, zIndex: 5 }}>
                No fresh canonical GPS session or authoritatively verified property pin is available. No markers have been fabricated.
              </Alert>
            )}
            {mapReady && (
