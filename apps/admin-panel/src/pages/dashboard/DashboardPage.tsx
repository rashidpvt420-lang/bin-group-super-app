...
const calculateMrr = (rows: RecordRow[]): MrrStats => {
    ...
    rows.filter(approvedPayment).forEach((row) => {
        ...
        else if (paidAt >= previousStart && paidAt < currentStart) previousMonth += amount;
    });
...
export default function DashboardPage() {
    ...
    useEffect(() => {
        ...
        listen('active tickets', query(collection(db, 'maintenanceTickets'), where('status', 'in', ACTIVE_TICKET_STATUSES)), (snap) => {
            const rows = snap.docs.map((row: any) => ({ id: row.id, ...(row.data() as Record<string, any>) })) as RecordRow[];
            rows.sort((a: RecordRow, b: RecordRow) => resolveSlaDueAt(a) - resolveSlaDueAt(b));
        ...
        listen('owner onboarding reviews', query(collection(db, 'intake_submissions'), where('status', 'in', OPEN_REVIEW_STATES), limit(10)), (snap) => {
            ...
            setApprovalQueue((current) => {
                ...
                return [...rows, ...techRows].sort((a: RecordRow, b: RecordRow) => getMillis(b.createdAt || b.submittedAt) - getMillis(a.createdAt || a.submittedAt));
            });
            setLastSync(new Date());
        });
        ...
        listen('technician reviews', query(collection(db, 'users'), where('role', '==', 'technician'), limit(100)), (snap) => {
            const rows = snap.docs
                .map((row: any) => ({ id: row.id, type: 'TECH_ONBOARD', origin: 'Technician onboarding', ...(row.data() as Record<string, any>) })) as RecordRow[];
            ...
            setApprovalQueue((current) => {
                ...
                return [...ownerRows, ...pendingRows].sort((a: RecordRow, b: RecordRow) => getMillis(b.createdAt || b.submittedAt) - getMillis(a.createdAt || a.submittedAt));
            });
            setLastSync(new Date());
        });
        ...
        listen('passport expiry scan', query(collection(db, 'property_passports'), limit(100)), (snap) => {
            const rows = snap.docs
                ...
                .filter(hasExpiredField);
            setExpiredDocuments((current) => [...current.filter((row) => row.source === 'documents'), ...rows]);
            setLastSync(new Date());
        });
        ...
        <AdminPageFrame title="Executive Command Center" subtitle="HARD-LIVE OPERATIONS TERMINAL" lastUpdated={lastSync} onRefresh={() => window.location.reload()}>
            <Stack spacing={4}>
                ...
                <Grid container spacing={3}>
                                    ...
                                    <TableBody>
                                        {actionQueue.map((item) => (
                                            <TableRow key={`${item.type}-${item.id}`} hover>
                                                ...
                                                <TableCell><Chip size="small" label={normalizeStatus(item.status || item.approvalStatus || item.verificationState)} sx={{ color: statusTone(item.status || item.approvalStatus || item.verificationState), bgcolor: alpha(statusTone(item.status || item.approvalStatus || item.verificationState), 0.12), fontWeight: 900 }} /></TableCell>
                    ...
                    <Grid item xs={12} lg={5}>
                        <Paper sx={{ p: 3, bgcolor: '#0f172a', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 5, height: '100%' }}>
                            <Typography variant="h6" sx={{ color: '#fff', fontWeight: 950, display: 'flex', gap: 1, alignItems: 'center', mb: 2 }}><Activity color={binThemeTokens.gold} /> SLA Breach Queue</Typography>
                            <Stack spacing={1.5}>
                ...
                <Grid container spacing={3}>
                    ...
                    <Grid item xs={12} lg={6}>
                        <Paper sx={{ p: 3, bgcolor: '#0f172a', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 5, height: '100%' }}>
                            <Typography variant="h6" sx={{ color: '#fff', fontWeight: 950, display: 'flex', gap: 1, alignItems: 'center', mb: 2 }}><Shield color={binThemeTokens.gold} /> 5-Profile Smoke Status</Typography>
                            <Grid container spacing={1.5}>
                                {smokeRows.map(([label, value]) => (
                                    <Grid item xs={12} sm={6} key={label}>
                                        <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ p: 1.5, bgcolor: 'rgba(255,255,255,0.04)', borderRadius: 2 }}>
                                            <Typography variant="body2" sx={{ color: '#fff', fontWeight: 800 }}>{label}</Typography>
                                            <Chip size="small" label={normalizeStatus(value)} sx={{ bgcolor: alpha(statusTone(value), 0.12), color: statusTone(value), fontWeight: 900 }} />
                ...
                <Grid container spacing={3}>
                    <Grid item xs={12} lg={4}>
                        <Paper sx={{ p: 3, bgcolor: '#0f172a', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 5, height: '100%' }}>
                            ...
                            <Stack spacing={1.5}>
                                <Box><Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.45)', fontWeight: 900 }}>CURRENT MONTH COLLECTIONS</Typography><Typography sx={{ color: '#fff', fontWeight: 950 }}>{money(mrrStats.currentMonth)}</Typography></Box>
                                <Box><Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.45)', fontWeight: 900 }}>PREVIOUS MONTH</Typography><Typography sx={{ color: 'rgba(255,255,255,0.72)', fontWeight: 950 }}>{money(mrrStats.previousMonth)}</Typography></Box>
                                <Box><Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.45)', fontWeight: 900 }}>MONTH TREND</Typography><Typography sx={{ color: mrrStats.trendPercent >= 0 ? '#10b981' : '#ef4444', fontWeight: 950 }}>{mrrStats.trendPercent.toFixed(1)}%</Typography></Box>
                            </Stack>
                        </Paper>
                    </Grid>
                    ...
                    <Grid item xs={12} lg={4}>
                        <Paper sx={{ p: 3, bgcolor: '#0f172a', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 5, height: '100%' }}>
                            ...
                            <Stack spacing={1.5}>
                                <Stack direction="row" justifyContent="space-between"><Typography sx={{ color: 'rgba(255,255,255,0.62)', fontWeight: 800 }}>Status</Typography><Chip size="small" label={normalizeStatus(webhook.status || webhook.state || healthValue('whatsappWebhook'))} sx={{ bgcolor: alpha(statusTone(webhook.status || webhook.state || healthValue('whatsappWebhook')), 0.12), color: statusTone(webhook.status || webhook.state || healthValue('whatsappWebhook')), fontWeight: 900 }} /></Stack>
                                <Stack direction="row" justifyContent="space-between"><Typography sx={{ color: 'rgba(255,255,255,0.62)', fontWeight: 800 }}>Signature</Typography><Typography sx={{ color: '#fff', fontWeight: 900 }}>{normalizeStatus(webhook.signatureStatus || webhook.signature || 'UNKNOWN')}</Typography></Stack>
                                <Stack direction="row" justifyContent="space-between"><Typography sx={{ color: 'rgba(255,255,255,0.62)', fontWeight: 800 }}>Error Count</Typography><Typography sx={{ color: Number(webhook.errorCount || 0) ? '#ef4444' : '#10b981', fontWeight: 900 }}>{Number(webhook.errorCount || 0)}</Typography></Stack>
