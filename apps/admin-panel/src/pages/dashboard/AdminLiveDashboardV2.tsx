...
import { Box, Button, Chip, Grid, Paper, Stack, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Typography, alpha } from '@mui/material';
...
type Card = { label: string; value: string | number; path: string; icon: React.ReactNode; tone: string };
...
const reviewPath = (item: Row) => {
  if (item.type === 'OWNER_ONBOARDING') return '/properties/approvals';
...
export default function AdminLiveDashboardV2() {
  ...
  useEffect(() => {
    ...
    const listen = (name: string, source: any, handler: (snap: any) => void) => {
      ...
      }, (error: any) => setStreamErrors((old) => ({ ...old, [name]: error?.code || error?.message || 'failed' })));
  ...
  const cards: Card[] = useMemo(() => [
    ...
    { label: 'Property Approvals', value: ownerApprovals.length, path: '/properties/approvals', icon: <Shield size={20} />, tone: ownerApprovals.length ? binThemeTokens.warning : '#10b981' },
    { label: 'Staff Access', value: 'Open', path: '/staff-access', icon: <Users size={20} />, tone: '#22c55e' },
    { label: 'Units', value: totalUnits, path: '/admin/unit-status', icon: <Building2 size={20} />, tone: '#60a5fa' },
    ...
    { label: 'Contracts', value: contracts, path: '/contracts/termination', icon: <FileText size={20} />, tone: binThemeTokens.gold },
    ...
    { label: 'Audit Today', value: auditToday, path: '/audit', icon: <Activity size={20} />, tone: binThemeTokens.gold },
