...
export default function DataGovernanceAuditPage() {
  ...
  React.useEffect(() => {
    ...
    return onSnapshot(
      ...
      (error: any) => {
        console.error('[DataGovernanceAudit] listener failed:', error);
        setLoadError(error?.message || 'Unable to load governance events.');
        setLoading(false);
      },
    );
  }, []);
  ...
  return (
    <Box sx={{ p: 4, color: '#fff' }}>
      ...
      {loadError && <Alert sx={{ mb: 3 }} severity="error">{loadError}</Alert>}
      {notice && <Alert sx={{ mb: 3 }} severity={notice.includes('Failed') ? 'error' : notice.includes('required') ? 'warning' : 'success'}>{notice}</Alert>}
      ...
      {loading ? (
        <Box role="status" sx={{ py: 8, display: 'grid', placeItems: 'center' }}><CircularProgress /></Box>
      ) : events.length === 0 ? (
