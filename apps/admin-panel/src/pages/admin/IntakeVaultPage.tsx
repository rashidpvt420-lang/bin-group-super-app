                    ...
                    <TableBody>
                        {loading ? <TableRow><TableCell colSpan={5} align="center" sx={{ py: 8 }}><CircularProgress sx={{ color: GOLD }} /></TableCell></TableRow> : submissions.length === 0 ? <TableRow><TableCell colSpan={5} align="center" sx={{ py: 8, color: 'rgba(255,255,255,0.5)' }}>No Owner applications found.</TableCell></TableRow> : submissions.map((item) => {
                            ...
                            return <TableRow key={item.id} hover sx={{ '& td': { borderColor: 'rgba(255,255,255,0.05)' } }}>
                                ...
                                <TableCell align="right"><Stack direction="row" spacing={1} justifyContent="flex-end" flexWrap="wrap" useFlexGap><IconButton aria-label="View owner intake" onClick={() => setSelected(item)} sx={{ color: '#FFF' }}><Eye size={18} /></IconButton><IconButton aria-label="Message owner" onClick={() => void messageOwner(item)} disabled={busyId === item.id} sx={{ color: GOLD }}><Mail size={18} /></IconButton>{actionButton(item)}</Stack></TableCell>
                            </TableRow>;
                        })}
                    </TableBody>
                </Table>
