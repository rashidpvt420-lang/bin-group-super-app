...
export default function AdminPaymentApproval() {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      const response: any = await approvePayment({
      ...
      alert(error?.message || t('admin.failed_verify_settlement'));
    } finally {
    ...
    <Box sx={{ p: 4, bgcolor: '#020617', minHeight: '100vh', color: 'white', direction: isRTL ? 'rtl' : 'ltr' }}>
      <Stack direction={isRTL ? 'row-reverse' : 'row'} spacing={2} alignItems="center" sx={{ mb: 4 }}>
        ...
        <Box sx={{ textAlign: isRTL ? 'right' : 'left' }}>
          <Typography variant="h4" sx={{ fontWeight: 900, letterSpacing: -1 }}>{t('admin.settlement_vault')}</Typography>
          <Typography variant="body2" sx={{ color: '#94a3b8', fontWeight: 700 }}>{t('admin.payment_authority')}</Typography>
        </Box>
      </Stack>
      ...
      <Grid container spacing={4} sx={{ flexDirection: isRTL ? 'row-reverse' : 'row' }}>
                ...
                {pendingContracts.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={5} align="center" sx={{ py: 10 }}>
                      <Stack alignItems="center" spacing={2}>
                        ...
                        <Typography variant="body2" sx={{ color: '#64748b', fontStyle: 'italic' }}>{t('admin.queue_clear')}</Typography>
                      </Stack>
                    </TableCell>
                  </TableRow>
                  ...
                  <TableRow key={contract.id} hover sx={{ '&:last-child td, &:last-child th': { border: 0 }, '&:hover': { bgcolor: 'rgba(255,255,255,0.02)' }, flexDirection: isRTL ? 'row-reverse' : 'row' }}>
                    <TableCell sx={{ textAlign: isRTL ? 'right' : 'left' }}>
                      <Typography variant="body2" sx={{ fontWeight: 900, color: '#fff', fontFamily: 'monospace' }}>{contract.paymentId}</Typography>
                      <Typography variant="caption" sx={{ color: '#64748b' }}>{t('onboarding.quote')}: {contract.id.slice(0, 8)}...</Typography>
                    </TableCell>
                    <TableCell sx={{ textAlign: isRTL ? 'right' : 'left' }}>
                      <Chip label={contract.provider} size="small" sx={{ bgcolor: alpha('#C6A75E', 0.1), color: '#C6A75E', fontWeight: 900, border: '1px solid #C6A75E' }} />
                    </TableCell>
                    <TableCell align={isRTL ? 'left' : 'right'}>
                      <Typography variant="body1" sx={{ color: '#10b981', fontWeight: 900 }}>{t('common.currency_aed')} {Number(contract.amount || 0).toLocaleString(lang === 'ar' ? 'ar-AE' : 'en-AE')}</Typography>
                    </TableCell>
                    <TableCell sx={{ textAlign: isRTL ? 'right' : 'left' }}>
                      <Typography variant="body2" sx={{ color: '#cbd5e1', fontWeight: 700 }}>{t('common.asset')}: {contract.ownerId?.slice(0, 8) || 'N/A'}...</Typography>
                    </TableCell>
                    <TableCell align={isRTL ? 'left' : 'right'}>
                      <Button
                        variant="contained"
        ...
        <Grid item xs={12} lg={3}>
          <Paper sx={{ p: 3, bgcolor: '#0f172a', border: '1px solid #1e293b', borderRadius: 4 }}>
            ...
            <Typography variant="body2" sx={{ color: '#94a3b8', mb: 2, textAlign: isRTL ? 'right' : 'left' }}>{t('admin.policy_1')}</Typography>
            <Typography variant="body2" sx={{ color: '#94a3b8', mb: 2, textAlign: isRTL ? 'right' : 'left' }}>{t('admin.policy_2')}</Typography>
            <Typography variant="body2" sx={{ color: '#94a3b8', mb: 2, textAlign: isRTL ? 'right' : 'left' }}>{t('admin.policy_3')}</Typography>
            <Box sx={{ mt: 4, p: 2, bgcolor: alpha('#ef4444', 0.1), border: '1px solid #ef4444', borderRadius: 2 }}>
              <Typography variant="caption" sx={{ color: '#ef4444', fontWeight: 900, display: 'block', textAlign: 'center' }}>{t('admin.warning_audit')}</Typography>
            </Box>
          </Paper>
        </Grid>
      </Grid>

      <Dialog open={verifyDialogOpen} onClose={() => !processingId && setVerifyDialogOpen(false)} dir={isRTL ? 'rtl' : 'ltr'} PaperProps={{ sx: { bgcolor: '#0f172a', color: 'white', borderRadius: 4, border: '1px solid #1e293b', width: '100%', maxWidth: 500 } }}>
        <DialogTitle sx={{ fontWeight: 900, borderBottom: '1px solid #1e293b', textAlign: isRTL ? 'right' : 'left' }}>{t('admin.confirm_settlement')}</DialogTitle>
        <DialogContent sx={{ pt: 3 }}>
          <Stack spacing={3}>
            <Box sx={{ textAlign: isRTL ? 'right' : 'left' }}>
              <Typography variant="caption" sx={{ color: '#94a3b8', textTransform: 'uppercase', fontWeight: 900 }}>{t('admin.verif_ref_label')}</Typography>
              <TextField fullWidth required variant="outlined" value={referenceId} onChange={(e: any) => setReferenceId(e.target.value)} placeholder="e.g. UTN-123456789" sx={{ mt: 1, '& .MuiOutlinedInput-root': { color: 'white', bgcolor: '#020617', '& fieldset': { borderColor: '#1e293b' }, '&:hover fieldset': { borderColor: '#C6A75E' }, '&.Mui-focused fieldset': { borderColor: '#C6A75E' } } }} />
            </Box>
            <Box sx={{ textAlign: isRTL ? 'right' : 'left' }}>
              <Typography variant="caption" sx={{ color: '#94a3b8', textTransform: 'uppercase', fontWeight: 900 }}>{t('admin.confirmed_amount')}</Typography>
              <TextField fullWidth type="number" variant="outlined" value={amountReceived} onChange={(e: any) => setAmountReceived(Number(e.target.value))} sx={{ mt: 1, '& .MuiOutlinedInput-root': { color: 'white', bgcolor: '#020617', '& fieldset': { borderColor: '#1e293b' } } }} />
            </Box>
            <Box sx={{ textAlign: isRTL ? 'right' : 'left' }}>
              <Typography variant="caption" sx={{ color: '#94a3b8', textTransform: 'uppercase', fontWeight: 900 }}>{t('admin.audit_notes')}</Typography>
              <TextField fullWidth multiline rows={3} variant="outlined" value={notes} onChange={(e: any) => setNotes(e.target.value)} placeholder={t('admin.internal_recon')} sx={{ mt: 1, '& .MuiOutlinedInput-root': { color: 'white', bgcolor: '#020617', '& fieldset': { borderColor: '#1e293b' } } }} />
            </Box>
          </Stack>
        </DialogContent>
