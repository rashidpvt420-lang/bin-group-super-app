...
function requireCredentials() {
  const missing = [
    !BROKER_EMAIL ? 'E2E_BROKER_EMAIL' : '',
    !BROKER_PASSWORD ? 'E2E_BROKER_PASSWORD' : '',
    !OWNER_EMAIL ? 'E2E_OWNER_EMAIL' : '',
    !ADMIN_EMAIL ? 'E2E_ADMIN_EMAIL' : '',
    !ADMIN_PASSWORD ? 'E2E_ADMIN_PASSWORD' : '',
    !/^\d{6}$/.test(ADMIN_MFA_CODE) ? 'E2E_ADMIN_REAL_MFA_CODE' : '',
    !process.env.E2E_OWNER_PASSWORD ? 'E2E_OWNER_PASSWORD' : '',
    !process.env.VITE_FIREBASE_APPCHECK_DEBUG_TOKEN ? 'VITE_FIREBASE_APPCHECK_DEBUG_TOKEN' : '',
  ].filter(Boolean);
  if (missing.length) {
...
function payoutCard(page: Page, brokerName: string, amount: number, status: string) {
  const amountLabel = `AED ${Number(amount).toLocaleString()}`;
  return payoutPanel(page)
...
async function rejectFirstPayout(page: Page, evidence: any) {
  ...
  const card = payoutCard(page, evidence.broker.displayName, evidence.payout.first.amount, 'PENDING ADMIN REVIEW');
  ...
  await expect(payoutPanel(page)).toContainText('REJECTED', { timeout: 30_000 });
}

async function approveAndPayReplacement(page: Page, evidence: any) {
  await page.goto(adminUrl(`/broker?refresh=${Date.now()}`), { waitUntil: 'domcontentloaded' });
  const card = payoutCard(page, evidence.broker.displayName, evidence.payout.second.amount, 'PENDING ADMIN REVIEW');
  await expect(card).toBeVisible({ timeout: 30_000 });
...
test.describe('Broker Business Workflow', () => {
  ...
  test('Broker lead converts through Owner activation, one commission, verified OTP and Admin MFA settlement', async ({ page }, testInfo) => {
    requireCredentials();
