...
test.describe('Tenant launch audit', () => {
  ...
  for (const [title, route] of [
    ['tenant unit page loads', '/tenant/unit'],
    ['tenant tickets list loads', '/tenant/tickets'],
    ['tenant documents vault renders', '/tenant/documents'],
    ['tenant chat interface renders', '/tenant/chat'],
    ['tenant profile page renders', '/tenant/profile'],
  ...
  test('tenant AR/EN language switch works in shell', async ({ page }) => {
    ...
    await page.waitForTimeout(1_500);
    await assertHealthy(page, '/tenant/dashboard');
    const langBtn = page.getByTestId('tenant-language-toggle');
    ...
    await expect.poll(() => new URL(page.url()).pathname).toBe('/tenant/dashboard');
    await page.getByTestId('tenant-language-toggle').click();
