...
test('[frozen-workflow] complete-batch binding preflight runs before production access', async () => {
  ...
  assert.match(workflow, /verify\/all\|verify\/paymentUnlockExactlyOnce\|verify\/brokerCommissionLockExactlyOnce/);
  assert.match(workflow, /sync\/paymentUnlockExactlyOnce\|repair-and-sync\/paymentUnlockExactlyOnce/);
