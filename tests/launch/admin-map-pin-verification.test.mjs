...
test('Admin map defines one fail-closed verified property-pin contract', () => {
  ...
  assert.match(source, /metadata\.dispatchReady === true \|\| ticket\?\.dispatchGeoReady === true/);
  ...
  assert.match(source, /metadata\.immutableSnapshot === true/);
  assert.match(source, /metadata\.canonicalPropertyId/);
