...
test('matching active STOP applies and an acknowledged duplicate is idempotent', () => {
  assert.equal(cas.classifyStopRequest(state(), 'ticket-1', 'session-A'), 'APPLY');
  assert.equal(cas.classifyStopRequest(
...
test('duplicate STOP requires the same recorded stopped ticket', () => {
  const stopped = state({ isTracking: false, activeTicketId: '', lastStoppedTicketId: 'ticket-1' });
  assert.equal(cas.classifyStopRequest(stopped, 'ticket-2', 'session-A'), 'REJECT_SUPERSEDED');
  assert.equal(cas.classifyStopRequest(
    { ...stopped, lastStoppedTicketId: '' },
    'ticket-1',
    'session-A',
  ), 'REJECT_SUPERSEDED');
});
...
test('watchdog cannot clear a superseding session, changed ticket, stopped state or missing record', () => {
  const queried = state();
  assert.equal(cas.classifyWatchdogCandidate(
    queried,
    state({ trackingSessionId: 'session-B' }),
    2_000,
  ), 'SKIP_SESSION_SUPERSEDED');
  assert.equal(cas.classifyWatchdogCandidate(
    queried,
    state({ activeTicketId: 'ticket-2' }),
    2_000,
  ), 'SKIP_TICKET_CHANGED');
  assert.equal(cas.classifyWatchdogCandidate(
    queried,
...
test('server implementation performs STOP, UPDATE and watchdog comparisons inside transactions', () => {
