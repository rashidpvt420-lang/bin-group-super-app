...
test('generic Admin browser fallback excludes properties for create and update', () => {
  ...
  assert.match(catchall, /allow create:[\s\S]*'system_secrets',\s*'technician_live_locations',\s*'properties',\s*'users'/);
  assert.match(catchall, /allow update, delete:[\s\S]*'system_secrets',\s*'technician_live_locations',\s*'properties',\s*'users'/);
});

test('canonical Firestore hardener migrates and forbids every prior properties-writable list', () => {
