...
test('Admin MFA remediation inventory is App Check protected, read-only and masked', async () => {
  ...
  assert.match(source, /READINESS_VIEWER_ROLES = new Set\(\["ceo", "super_admin"\]\)/);
  assert.match(source, /CEO or Super Admin authority is required/);
