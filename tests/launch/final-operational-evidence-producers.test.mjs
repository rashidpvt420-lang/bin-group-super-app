...
test('technician physical evidence is protected and requires real mobile, GPS and Storage proof', async () => {
  ...
  assert.match(verifier, /onSiteVerification/);
  assert.match(verifier, /accuracy <= 100/);
  assert.match(verifier, /bucket\.file\(objectPath\)\.exists\(\)/);
...
test('privileged rotation evidence is provider-backed and requires revoked previous credentials', async () => {
  const [workflow, verifier] = await Promise.all([
  ...
  assert.match(verifier, /STRIPE_SECRET_KEY/);
  assert.match(verifier, /STRIPE_WEBHOOK_SECRET/);
  assert.match(verifier, /SMTP_PASS/);
