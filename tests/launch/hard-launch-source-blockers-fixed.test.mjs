...
const accountCreation = read('src/components/onboarding/AccountCreationStep.tsx');
...
test('Broker referral link reaches public onboarding and locks attribution server-side for a verified Owner', () => {
  ...
  assert.match(onboarding, /<AccountCreationStep[^>]*brokerUid=\{brokerUid\}/);
  assert.doesNotMatch(onboarding, /captureBrokerReferralAttribution/);
  assert.match(accountCreation, /getIdTokenResult\(true\)/);
  ...
  assert.match(brokerCallable, /tokenRole !== "owner"/);
  assert.match(brokerCallable, /currentRole !== "owner"/);
