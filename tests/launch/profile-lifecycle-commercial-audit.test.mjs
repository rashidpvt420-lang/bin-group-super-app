...
test('Tenant routing exposes renewal and both inspection variants', async () => {
  ...
  expectAll(source, [/path=["']\/renewals["']/, /path=["']\/move-inspection["']/, /path=["']\/move-inspection\/:type["']/], 'Tenant lifecycle routes');
});
...
test('Broker KYC callable binds current terms version and private bank ownership fields', async () => {
  ...
  expectAll(page, [/BIN_BROKER_TERMS_2026_01/, /commissionAgreementAccepted/, /bankAccountHolder/, /bankIban/, /submitBrokerKycProfile/], 'Broker profile submission');
...
test('Onboarding review maps canonical service strategies without combined-plan fallback', async () => {
  const source = await read('src/components/onboarding/ReviewBeforeSubmitStep.tsx');
  expectAll(source, [
    /reviewPlanKeyForStrategy/,
    /strategy === ['"]fm_only['"] \|\| strategy === ['"]fm['"]/,
    /strategy === ['"]pm_only['"] \|\| strategy === ['"]rent['"]/,
    /return ['"]ifm['"]/,
    /planKey === ['"]amc['"]/,
    /planKey === ['"]pm['"]/,
    /Property Management Only/,
  ], 'Canonical contract plan mapping');
...
test('Arabic contracts exist on payment, onboarding and role-profile surfaces', async () => {
  const paths = ['src/components/onboarding/PaymentSummaryStep.tsx', 'src/pages/PropertyOnboardingPage.tsx', 'src/owner/pages/OwnerProfilePage.tsx', 'src/tenant/pages/TenantProfilePage.tsx', 'src/technician/pages/TechnicianProfilePage.tsx', 'src/broker/pages/BrokerProfilePage.tsx'];
