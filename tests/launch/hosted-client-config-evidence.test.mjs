...
const ENV = {
  ...
  REACT_APP_FIREBASE_API_KEY: `AIza${'A'.repeat(35)}`,
  ...
  REACT_APP_FIREBASE_MESSAGING_SENDER_ID: '987654321000',
...
function adminTexts() {
  return [
    ...
    `${ENV.REACT_APP_FIREBASE_API_KEY} ${ENV.REACT_APP_ADMIN_FIREBASE_APP_ID} ${ENV.REACT_APP_FIREBASE_MESSAGING_SENDER_ID} ${ENV.REACT_APP_APP_CHECK_SITE_KEY}`,
  ];
}

function adminTextsWithPublicRuntimeValues() {
  return [
    `bin-group-57c60 bin-group-57c60.firebaseapp.com bin-group-57c60.firebasestorage.app`,
    `${ENV.VITE_FIREBASE_API_KEY} ${ENV.REACT_APP_ADMIN_FIREBASE_APP_ID} ${ENV.VITE_FIREBASE_MESSAGING_SENDER_ID} ${ENV.VITE_APP_CHECK_SITE_KEY}`,
  ];
}
...
test('hosted client summaries require exact main and Admin runtime values', () => {
  ...
  assert.equal(admin.firebaseApiKeyMatched, true);
  assert.equal(admin.messagingSenderIdMatched, true);
...
test('Admin hosted verification cannot be satisfied by public-app runtime values', () => {
  const admin = summarizeHostedClientBundle({
    texts: adminTextsWithPublicRuntimeValues(),
    assetCount: 2,
    site: 'admin',
    env: ENV,
  });
  assert.equal(admin.firebaseAppIdMatched, true);
  assert.equal(admin.firebaseApiKeyMatched, false);
  assert.equal(admin.messagingSenderIdMatched, false);
  assert.equal(admin.appCheckSiteKeyMatched, false);
