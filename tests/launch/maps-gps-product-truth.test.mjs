...
test('Canonical property verification contract is fail-closed and metadata-complete', () => {
  ...
  assert.match(verifiedPinContract, /const founderVerified =/);
  ...
  assert.match(verifiedPinContract, /const physicalVerified =/);
  assert.match(verifiedPinContract, /geo\.source === 'physical_inspection'/);
  assert.match(verifiedPinContract, /verification\.source === 'PHYSICAL_INSPECTION_EVIDENCE'/);
  assert.match(verifiedPinContract, /evidenceHash/);
  assert.match(verifiedPinContract, /evidenceGeneration/);
  assert.match(verifiedPinContract, /text\(geo\.inspectionId, 240\) === inspectionId/);
  assert.match(verifiedPinContract, /geoVersion !== verificationVersion/);
  ...
  assert.match(verifiedPinContract, /if \(!founderVerified && !physicalVerified\) return null/);
