...
test('Broker commission creation is deterministic and has a contract-activation reconciliation trigger', () => {
  assert.match(brokerCommissions, /doc\(`commission_\$\{contractId\}`\)/);
  assert.match(brokerCommissions, /reconcileBrokerCommissionOnContractActivation = onDocumentUpdated/);
  assert.match(brokerCommissions, /after\.commissionGenerated !== true/);
