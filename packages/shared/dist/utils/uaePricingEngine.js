export const calculateUAEValuation = async (inputs) => {
    ...
    const gradeMultipliers = { 'Ultra-Luxury': 2.50, 'Luxury': 1.80, 'Premium': 1.30, 'Standard': 1.0, 'Sovereign': 3.0 };
    let sectorMult = 1.0;
    if (pType === 'GOVERNMENT_MAJLIS')
        ...
        sectorMult = 2.50;
    else if (['Commercial', 'Office', 'Warehouse', 'Mall'].includes(pType))
        sectorMult = 1.25;
    else if (['Institutional', 'School', 'Hospital'].includes(pType))
        sectorMult = 1.40;
    ...
    if (pType === 'GOVERNMENT_MAJLIS')
        maintenanceBase = Math.max(maintenanceBase, 35 * sqft);
    else if (pType === 'GOVERNMENT_PROPERTY')
        maintenanceBase = Math.max(maintenanceBase, 25 * sqft);
    else if (pType === 'HOTEL')
        maintenanceBase = Math.max(maintenanceBase, 30 * sqft);
