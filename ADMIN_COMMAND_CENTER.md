...
Initial founder/Admin recovery must use the canonical protected Firebase Production Deploy workflow in bank-pilot mode with the `ADMIN_MFA_BOOTSTRAP_HOSTING` marker and the runbooks under `docs/launch/`. It must not be performed by editing Firestore, setting custom claims locally, creating passwords with workstation scripts, or running a parallel partial deployment.
...
Production changes are deployed only through `.github/workflows/firebase-production-deploy.yml` from the exact current `main` SHA. Local or parallel partial Firebase deployments are prohibited, including emergency recovery attempts.
