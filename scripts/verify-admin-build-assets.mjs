...
for (const source of scriptSources) {
  ...
  if (relativeCheck.startsWith('..') || path.isAbsolute(relativeCheck)) {
    fail(`script source escapes the Admin build directory: ${source}`);
  }
  ...
  if (/^\s*(?:<!doctype\s+html|<html\b)/i.test(content)) {
    fail(`Admin JavaScript asset contains HTML fallback content: ${assetPath}`);
  }
...
const requiredFirebaseMarkers = [
  'bin-group-57c60',
  'bin-group-57c60.firebaseapp.com',
  '1:123413252227:web:285cb53bc26626d699f3b6',
];
for (const marker of requiredFirebaseMarkers) {
...
if (productionAppCheckRequired) {
  if (!siteKey || /REPLACE|PLACEHOLDER|process\.env/i.test(siteKey) || siteKey.length < 20) {
    fail('protected production build requested App Check but no plausible Admin site key was supplied');
  }
  if (!mergedJavaScript.includes(siteKey)) {
    fail('protected production App Check site key was not embedded in the Admin bundle');
  }
