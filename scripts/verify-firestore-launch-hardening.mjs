...
const forbiddenFragments = [
  ...
  ['invoice registry exposed through global read fallback', "allow read: if collection != 'tickets' && collection != 'maintenanceTickets' && !(collection in ['system_secrets', 'users', 'broker_kyc_submission_limits', 'admin_security_sessions', 'private_hr_profiles', 'technician_live_locations', 'invoice_registry']) && hasAdminClaim();"],
...
const requiredFragments = [
  ...
  ['ticket write fallback excludes explicit ticket hierarchies, live location, canonical property geo, private HR and HR cases', "'system_secrets',\n          'technician_live_locations',\n          'properties',\n          'users',\n          'audit_logs',\n          'admin_security_sessions',\n          'private_hr_profiles',\n          'staffRequests',\n          'hrAiConversations'"],
