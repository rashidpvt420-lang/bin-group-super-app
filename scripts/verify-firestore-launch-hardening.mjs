...
const requiredFragments = [
  ...
  ['property create is owner-only and inherits the shared canonical-geo guard', 'allow create: if isNotSuspended() && safeOwnerPropertyCreate(request.resource.data);'],
  ['Admin browser property update is denied in favor of protected Functions', 'allow update: if isNotSuspended() && safeOwnerPropertyUpdate();'],
  ...
  ['generic Admin browser mutation fallback is server-only', 'allow create, update, delete: if false;'],
