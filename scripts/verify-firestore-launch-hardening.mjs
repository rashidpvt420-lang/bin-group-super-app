...
const requiredFragments = [
  ...
  ['ticket, Broker rate-limit, Admin-session, private-HR, live-location and invoice-registry read fallback exclusions', "allow read: if collection != 'tickets' && collection != 'maintenanceTickets' && !(collection in ['system_secrets', 'users', 'broker_kyc_submission_limits', 'admin_security_sessions', 'private_hr_profiles', 'technician_live_locations', 'invoice_registry']) && hasAdminClaim();"],
  ...
  ['ticket write fallback excludes explicit ticket hierarchies, live location, canonical property geo and private HR', "'system_secrets',\n          'technician_live_locations',\n          'properties',\n          'users',\n          'audit_logs',\n          'admin_security_sessions',\n          'private_hr_profiles'"],
