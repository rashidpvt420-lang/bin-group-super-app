 ...
 *   and records the real pass/fail count — this script does not assert "12/12"
 *   itself, it records what actually happened; postdeploy-release-gate.mjs is the
 *   one place that enforces the required count.
 *
 ...
 *   and sums them into one aggregate record. Does not re-run anything itself —
 *   run `npm run test:e2e:business` before this script.
 *
...
process.exit(gate11Ok && businessOk ? 0 : 1);
