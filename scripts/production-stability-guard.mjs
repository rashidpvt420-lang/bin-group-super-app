...
const phase1OwnerRepair = read('functions/phase1OwnerLaunchRepair.ts');
const inspectionLink = read('functions/ownerInspectionAdminLink.ts');
const inspectionEvidence = read('functions/phase1OwnerVisitEvidenceHardened.ts');
...
assert(accountStep.indexOf('setOwnerAccount({') > accountStep.indexOf('getIdTokenResult(true)'), 'Owner readiness must wait for refreshed verified Owner claims.');
...
assert(inspectionSubmission.includes('documentEvidence'), 'Page 5 must submit private immutable document evidence.');
assert(!inspectionSubmission.includes('downloadUrl') && !inspectionSubmission.includes('documentUrls'), 'Page 5 must not depend on permanent document URLs.');
...
assert(inspectionWorkflow.includes('previewOwnerInspectionQuote'), 'Backend must retain protected server quotation authority.');
assert(inspectionWorkflow.includes('requestOwnerInspectionSignatureOtp'), 'Backend must retain branded Owner signature OTP authority.');
assert(phase1OwnerRepair.includes('OWNER_FIVE_PAGE_INSPECTION_FIRST_V1'), 'Backend must version the repaired five-page workflow.');
assert(phase1OwnerRepair.includes('SUBMITTED_FOR_PROPERTY_INSPECTION'), 'Submission must enter Admin property-review state.');
assert(phase1OwnerRepair.includes('NOT_DUE_UNTIL_INSPECTION_COMPLETE'), '15% payment must not be due before verified visits.');
assert(phase1OwnerRepair.includes('VERIFIED_INSPECTION_EVIDENCE_REQUIRED_BEFORE_PAYMENT'), 'Payment record must fail closed until verified inspection evidence.');
assert(phase1OwnerRepair.includes('Number(quote.annualContractValue) * 0.15'), 'Backend must lock the exact 15% mobilisation amount.');
assert(phase1OwnerRepair.includes('canonicalPropertyId'), 'Backend must generate intake-scoped property IDs.');
assert(phase1OwnerRepair.includes('transaction.create(db.collection("properties")'), 'Canonical property records must use create-only writes.');
assert(phase1OwnerRepair.includes('adminRecordOwnerMobilizationPaymentEvidencePhase1'), 'Admin must record immutable Phase 1 receipt evidence before approval.');
assert(phase1OwnerRepair.includes('payment.inspectionVerified !== true'), 'Receipt recording must reject payment before verified visits.');
assert(phase1OwnerRepair.includes('paymentConfigVersion: configuration.version'), 'Receipt evidence must bind the active payment configuration version.');
assert(phase1OwnerRepair.includes('paymentConfigHash: configuration.configHash'), 'Receipt evidence must bind the active payment configuration hash.');
assert(phase1OwnerRepair.includes('new Set(["CASH", "CHEQUE"])'), 'Phase 1 payment must allow only Cash and Cheque.');
assert(!phase1OwnerRepair.includes('firebaseStorageDownloadTokens'), 'Sensitive Owner and payment evidence must not create permanent Storage download tokens.');
...
assert(inspectionEvidence.includes('adminRecordOwnerPortfolioVisitEvidenceHardened'), 'Admin must use the hardened evidence handler for every property visit.');
assert(inspectionEvidence.includes('MAX_ARRIVAL_DISTANCE_METRES = 250'), 'Visit evidence must enforce the 250 metre arrival radius.');
assert(inspectionEvidence.includes('MAX_ARRIVAL_ACCURACY_METRES = 100'), 'Visit evidence must reject GPS accuracy worse than 100 metres.');
assert(inspectionEvidence.includes('MAX_GPS_CAPTURE_AGE_MS = 2 * 60 * 1000'), 'Visit evidence must require a fresh GPS capture.');
assert(inspectionEvidence.includes('BROWSER_GEOLOCATION_AT_SUBMISSION'), 'Visit evidence must bind browser geolocation captured at submission.');
assert(inspectionEvidence.includes('gpsAccuracyVerified: true'), 'Visit evidence must record verified accuracy.');
assert(inspectionEvidence.includes('gpsCaptureFresh: true'), 'Visit evidence must record freshness.');
assert(inspectionEvidence.includes('verificationSource: "ADMIN_BROWSER_GPS_SITE_VISIT"'), 'Verified property pins must identify their Admin browser GPS source.');
assert(inspectionEvidence.includes('point: new admin.firestore.GeoPoint(verifiedLat, verifiedLng)'), 'Dispatch pins must use the captured Admin site coordinate.');
assert(inspectionEvidence.includes('checklistComplete: true'), 'Visit evidence must require a complete checklist.');
assert(inspectionEvidence.includes('photoCount: 1'), 'Visit evidence must require a property photo.');
assert(inspectionEvidence.includes('adminCompleteOwnerPortfolioInspectionsPhase1Hardened'), 'Portfolio completion must use the hardened evidence-backed handler.');
assert(inspectionEvidence.includes('PENDING_ADMIN_PAYMENT_VERIFICATION'), 'Verified visits must make the exact 15% ready for Admin verification.');
assert(adminFirebase.includes('captureFreshAdminBrowserGps'), 'Admin callable wrapper must capture current browser GPS at submission.');
assert(adminFirebase.includes("name === 'adminRecordOwnerPortfolioVisitEvidence'"), 'Only the Owner visit evidence callable may receive the automatic GPS binding.');
assert(adminFirebase.includes('arrivalAccuracyMetres'), 'Admin GPS binding must send measured accuracy.');
assert(adminFirebase.includes('positionCapturedAtMs'), 'Admin GPS binding must send the device capture timestamp.');
...
assert(runtime.includes('submitOwnerInspectionFirstOnboardingPhase1 as submitOwnerInspectionFirstOnboarding'), 'Runtime must explicitly deploy repaired five-page Owner submission.');
assert(runtime.includes('adminRecordOwnerMobilizationPaymentEvidencePhase1 as adminRecordOwnerMobilizationPaymentEvidence'), 'Runtime must explicitly deploy repaired immutable Owner payment evidence recording.');
assert(runtime.includes('adminRecordOwnerPortfolioVisitEvidenceHardened as adminRecordOwnerPortfolioVisitEvidence'), 'Runtime must explicitly deploy hardened Owner site evidence recording.');
assert(runtime.includes('adminCompleteOwnerPortfolioInspectionsPhase1Hardened as adminCompleteOwnerPortfolioInspections'), 'Runtime must explicitly deploy hardened evidence-backed portfolio completion.');
assert(runtime.includes('from "./phase1OwnerLaunchRepair"'), 'Runtime must source repaired Phase 1 Owner callables.');
assert(runtime.includes('from "./phase1OwnerVisitEvidenceHardened"'), 'Runtime must source hardened Admin site GPS callables.');
...
assert(!runtime.includes('export * from "./ownerInspectionCompletion";'), 'Runtime must not deploy notes-only portfolio completion.');
