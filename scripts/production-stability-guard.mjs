...
const inspectionEvidence = read('functions/ownerInspectionEvidence.ts');
...
assert(inspectionEvidence.includes('adminRecordOwnerPortfolioVisitEvidence'), 'Admin must record evidence for every property visit.');
assert(inspectionEvidence.includes('MAX_ARRIVAL_DISTANCE_METRES = 750'), 'Visit evidence must enforce a bounded arrival radius.');
...
assert(inspectionEvidence.includes('adminCompleteOwnerPortfolioInspectionsPhase1'), 'Portfolio completion must use the evidence-backed Phase 1 handler.');
...
assert(runtime.includes('adminCompleteOwnerPortfolioInspectionsPhase1 as adminCompleteOwnerPortfolioInspections'), 'Runtime must explicitly deploy evidence-backed portfolio completion.');
assert(runtime.includes('from "./phase1OwnerLaunchRepair"'), 'Runtime must source repaired Phase 1 Owner callables.');
assert(runtime.includes('from "./ownerInspectionEvidence"'), 'Runtime must source evidence-backed Admin visit callables.');
