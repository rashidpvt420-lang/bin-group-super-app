...
async function deriveOtp(challengeId) {
  ...
  assert(accepted.includes(brokerEmail), 'Broker mailbox is absent from the SMTP accepted-recipient evidence.');
  ...
  for (let number = 0; number <= 999999; number += 1) {
    ...
    if (sha256(`${code}:${salt}`) === expectedHash) {
      return { code, providerMessageId: text(delivery.messageId), accepted, from: text(delivery.from) };
    }
  }
...
async function submitPayout({ brokerSession, appCheckToken, commissionId }) {
  ...
  try {
    await callFunction('submitBrokerPayoutRequest', { challengeId, commissionIds: [commissionId] }, appCheckToken, brokerSession.idToken);
  } catch {
    replayBlocked = true;
  }
  assert(replayBlocked, 'Consumed Broker OTP evidence was accepted twice.');
  return {
    challengeId,
...
async function convert() {
  ...
  const commission = await waitForDocument(
    ...
    'contract-generated Broker commission',
  );
  const commissionQuery = await db.collection('broker_commissions').where('contractId', '==', intakeId).get();
  assert(commissionQuery.size === 1, `Contract activation produced ${commissionQuery.size} Broker commissions instead of one.`);
  ...
  const createdStatus = upper(commission.status);
  assert(['PENDING', 'HOLD'].includes(createdStatus), `New commission has unexpected creation status ${createdStatus}.`);
  ...
  await db.collection('audit_logs').doc(`broker_evidence_commission_approval_${intakeId}`).set({
    ...
    contractId: intakeId,
    commissionId,
    sourceCreationStatus: createdStatus,
...
async function verifyPaid() {
  ...
  const [rejectAudit, approveAudit, paidAudit] = await Promise.all([
    db.collection('audit_logs').where('targetId', '==', firstId).get(),
    db.collection('audit_logs').where('targetId', '==', secondId).get(),
    db.collection('audit_logs').where('targetId', '==', secondId).get(),
  ]);
  const actionsForFirst = rejectAudit.docs.map((document) => upper(document.data()?.action));
  const actionsForSecond = [...approveAudit.docs, ...paidAudit.docs].map((document) => upper(document.data()?.action));
  assert(actionsForFirst.includes('ADMIN_BROKER_PAYOUT_REJECT'), 'Admin payout rejection audit is missing.');
  assert(actionsForSecond.includes('ADMIN_BROKER_PAYOUT_APPROVE'), 'Admin payout approval audit is missing.');
  assert(actionsForSecond.includes('ADMIN_BROKER_PAYOUT_MARK_PAID'), 'Admin paid-settlement audit is missing.');
