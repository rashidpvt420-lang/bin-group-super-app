...
update('.github/workflows/firebase-production-deploy.yml', (source) => {
  const before = [
    "      E2E_BROKER_PASSWORD: ${{ secrets.E2E_BROKER_PASSWORD }}",
    "      E2E_STRICT_ROLES: 'true'",
  ...
  const after = [
    ...
    "      E2E_STRICT_ROLES: 'true'",
    "      E2E_STRICT_LIVE: 'true'",
    "      E2E_STRICT_BUSINESS: 'true'",
  ].join('\n');
  return replaceExact(source, before, after, 2, 'Founder production evidence wiring');
});
...
update('src/technician/pages/TechnicianJobDetailPage.tsx', (source) => {
  ...
  source = replaceExact(source, queueMarker, helperAndQueue, 1, 'Technician network error classifier');

  const acceptCatchBefore = [
    '        } catch (err: any) {',
    "            queueAction('ACCEPTED', err?.message || 'Accept mission failed before confirmation.');",
    '        } finally {',
  ].join('\n');
  const acceptCatchAfter = [
    '        } catch (err: any) {',
  ...
  source = replaceExact(source, acceptCatchBefore, acceptCatchAfter, 1, 'Visible online acceptance failure');

  const lifecycleCatchBefore = [
  ...
  return replaceExact(source, lifecycleCatchBefore, lifecycleCatchAfter, 1, 'Visible online lifecycle failure');
});
...
test('production evidence maps protected Founder MFA credentials into both production jobs', () => {
  for (const name of ['E2E_FOUNDER_EMAIL', 'E2E_FOUNDER_PASSWORD', 'E2E_FOUNDER_TOTP_SECRET', 'E2E_FOUNDER_REAL_MFA_CODE']) {
    assert.ok((workflow.match(new RegExp(name + ': \\\\$\\{\\{ secrets\\.' + name + ' \\}\\}', 'g')) || []).length >= 2, name);
  }
  assert.ok((workflow.match(/E2E_STRICT_BUSINESS: 'true'/g) || []).length >= 2);
});

test('protected five-role fixture makes the Technician callable-ready before Tenant handoff', () => {
  for (const field of ['currentShiftId', 'deviceRegistered', 'medicalCardStatus', 'drivingLicenseStatus', 'certificationsStatus', 'lastGpsAt', 'maxConcurrentJobs']) {
    assert.ok(fixture.includes(field), field);
  }
  assert.ok(fixture.includes("E2E_TECHNICIAN_EMAIL is required"));
