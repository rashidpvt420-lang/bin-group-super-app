...
if (process.env.GITHUB_ACTOR !== AUTOMATION_ACTOR) fail('automated child evidence must be dispatched by github-actions[bot]');
...
const sourceActorTrusted = authorizedActors.includes(sourceActor) || sourceActor === AUTOMATION_ACTOR;
...
if (currentMainSha !== expectedSha) fail(`main advanced from deployed SHA ${expectedSha} to ${currentMainSha}`);
if (!sourceActorTrusted) fail('source production deployment actor is neither an authorized founder nor the protected dispatcher');
...
const proof = {
  ...
  sourceActor,
  sourceActorTrust: authorizedActors.includes(sourceActor) ? 'authorized-founder' : 'protected-dispatch-bot',
