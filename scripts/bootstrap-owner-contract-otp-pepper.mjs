...
export function isValidOwnerContractOtpPepper(value) {
...
function describeSecret({ env = process.env } = {}) {
  ...
  return classifyOwnerSecretAccessFailure(`${result.stderr || ''}\n${result.stdout || ''}`);
}
...
function accessSecret({ env = process.env } = {}) {
  ...
  return {
    status: classifyOwnerSecretAccessFailure(`${result.stderr || ''}\n${result.stdout || ''}`),
    value: '',
  };
}
  ...
  try {
    ...
    if (action !== 'unchanged') {
      ...
      if (
        ...
        !isValidOwnerContractOtpPepper(verified.value)
      ) {
