...
if (deployedAt.getTime() > now + CLOCK_SKEW_MS) fail('production deployment timestamp is in the future');
...
const candidates = snapshot.docs
  ...
  .filter((entry) => entry.observedMs >= deployedAt.getTime() - CLOCK_SKEW_MS && entry.observedMs <= now + CLOCK_SKEW_MS)
...
const proof = {
  ...
  subjectHash: definition.subjectKind === 'staffUidHash' ? sha256(subject) : sha256(subject),
