 ...
 *
 * Hard-clearance may consume a same-run fresh production revalidation proof.
 * That proof is generated by a separate production-environment job so this
 * authorization job never needs to duplicate protected mailbox credentials.
 */
...
const consumeHardClearanceRevalidation =
  String(process.env.HARD_CLEARANCE_REVALIDATION_MODE || '').trim().toLowerCase() === 'consume';
...
const required = [
  ...
  ...(consumeHardClearanceRevalidation
    ? [{
        name: 'hardClearanceProductionRevalidation',
        cmd: 'node',
        args: ['scripts/hard-clearance-production-revalidation.mjs', '--verify'],
      }]
...
const status = {
  ...
  commitSha: sha,
  hardClearanceRevalidationConsumed: consumeHardClearanceRevalidation,
  automationOk,
  pilotEligible,
  hardLaunchEligible,
  hardLaunchClaim,
