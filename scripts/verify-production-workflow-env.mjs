...
if (launchMode === 'public') {
  ...
  if (String(process.env.PREDEPLOY_STRIPE_PROOF_OK || '') !== 'true') {
    failures.push('public launch mode requires PREDEPLOY_STRIPE_PROOF_OK=true');
  }
  if (String(process.env.POSTDEPLOY_STRIPE_LIVE_OK || '') !== 'true') {
    failures.push('public launch mode requires POSTDEPLOY_STRIPE_LIVE_OK=true');
  }
}
